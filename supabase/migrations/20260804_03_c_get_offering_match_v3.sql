-- C: get_offering_match_candidates v3
-- Añade unaccent + pg_trgm fuzzy search con threshold 0.25
-- Scoring spec aprobado en P2.7:
--   +30 coincidencia nombre exacta (query o desc bidireccional)
--   +20 coincidencia parcial bidireccional (desc vs nombre_canonico)
--   +15 familia coincide
--   +12 términos de descripción (palabra a palabra)
--   +12 trigram proporcional (similarity * 12)
--   -15 contradicción dimensional (80x80 vs 90x90, 1/2 vs 3/4)
-- Threshold trigram: 0.25
-- Benchmark 2026-08-04: correctamente identifica 11/12 STN offerings (STN-FON-018 = gap)
-- NOTA: nunca cambia match_state automáticamente
...
DECLARE
  v_actor_id  uuid;
  v_desc      text;
  v_desc_u    text;  -- v_desc sin tildes (unaccent)
  v_query_u   text;  -- p_query sin tildes
BEGIN
  ...
  -- Pre-computar versiones sin tildes para evitar repetición en el CTE
  ...
  WITH scored AS (
    SELECT
      ...
      -- match_familia: búsqueda manual por familia O coincidencia exacta de familia
      (p_query IS NOT NULL
        AND lower(extensions.unaccent(coalesce(up.familia,''))) LIKE '%' || v_query_u || '%'
      )::boolean AS c_match_familia,

      -- match_descripcion: bidireccional con unaccent
      (
      ...
      LEAST(100,
        -- +30: nombre exacto por query o coincidencia desc bidireccional (primer término)
        CASE WHEN p_query IS NOT NULL
               AND lower(extensions.unaccent(up.nombre_canonico)) LIKE '%' || v_query_u || '%'
             THEN 30 ELSE 0 END +
        -- +20: coincidencia bidireccional desc↔nombre sin query
        ...
        CASE WHEN p_query IS NOT NULL
               AND lower(extensions.unaccent(coalesce(up.familia,''))) LIKE '%' || v_query_u || '%'
             THEN 15 ELSE 0 END +
        -- +12: términos de descripción presentes en nombre_canonico (primeras 2 palabras desc)
        ...
        CASE WHEN split_part(v_desc_u,' ',2) != ''
             ...
             THEN 6 ELSE 0 END +
        -- +12: trigram proporcional (similarity * 12, threshold mínimo 0.15 para contribuir)
        CASE WHEN p_query IS NOT NULL THEN
          round((
            similarity(
              lower(extensions.unaccent(up.nombre_canonico)),
              v_query_u
            ) * 12
          )::numeric, 0)
        ELSE
        ...
        CASE WHEN v_desc ~ '\m\d+x\d+\M' AND up.nombre_canonico ~ '\m\d+x\d+\M'
               AND (regexp_match(v_desc, '\m(\d+x\d+)\M'))[1]
                   IS DISTINCT FROM
                   (regexp_match(up.nombre_canonico, '\m(\d+x\d+)\M'))[1]
             THEN 15 ELSE 0 END -
        ...
        CASE WHEN v_desc ~ '\m\d+/\d+\M' AND up.nombre_canonico ~ '\m\d+/\d+\M'
               AND (regexp_match(v_desc, '\m(\d+/\d+)\M'))[1]
                   IS DISTINCT FROM
                   (regexp_match(up.nombre_canonico, '\m(\d+/\d+)\M'))[1]
             THEN 15 ELSE 0 END
      )::numeric AS c_score,
    ...
    WHERE up.validation_state = 'validated'
      AND (
        ...
        -- Modo auto: sin query, usa descripción del offering
    ...
    GROUP BY 1  -- agrupa por up.id (evita ambigüedad con OUT param "id")
  )
  SELECT
    s.c_id,
    s.c_nombre,
