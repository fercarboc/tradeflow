...
COMMENT ON TABLE public.trade_catalog_import_chunks IS
  'Control de idempotencia por chunk. PK (import_id, chunk_index) garantiza unicidad.
   ...
   En reintento con hash diferente lanza CHUNK_HASH_MISMATCH.';
...
COMMENT ON TABLE public.trade_catalog_import_errors IS
  'Un registro por fila rechazada. Append-only.
   chunk_index = -1, fila_numero = -1 = error global del import.
   datos_fila truncado a ~2KB via _safe_datos_fila.';
...
BEGIN
  ...
  IF p_archivo_hash IS NULL OR trim(p_archivo_hash) = '' THEN
  ...
  INSERT INTO public.trade_catalog_imports (
    actor_id, nombre_archivo, archivo_hash, total_filas, chunk_size,
    chunks_esperados, mapping_config, parser_version, created_by
  ) VALUES (
    p_actor_id,
...
BEGIN
  ...
  IF v_import.archivo_hash <> p_archivo_hash THEN
    RAISE EXCEPTION 'IMPORT_HASH_MISMATCH: El hash del archivo no coincide con el import original. Original: %. Recibido: %.',
  ...
  IF FOUND THEN
    IF v_cached_hash <> p_chunk_hash THEN
      RAISE EXCEPTION
        ...
        USING ERRCODE = 'data_exception';
    END IF;
    RETURN jsonb_build_object('ok', v_cached_ok, 'errores', v_cached_err, 'cached', true);
  END IF;
  ...
  FOR v_idx IN 0..jsonb_array_length(p_items) - 1 LOOP
    ...
    BEGIN
      INSERT INTO public.trade_marketplace_supplier_offerings (
        ...
        true,
        'pending_review',
        jsonb_build_object('last_import_id', p_import_id, 'fila_original', v_fila_original, 'imported_at', now()),
        now()
      )
