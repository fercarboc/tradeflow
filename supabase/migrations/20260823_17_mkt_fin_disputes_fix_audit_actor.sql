-- ═══════════════════════════════════════════════════════════════════════════
-- MP-FIN-2C FIX 2/5: mkt_fin_audit 4th param auth.uid() → NULL
-- auth.uid() es un UUID de Supabase Auth, NO un trade_marketplace_actors.id
-- trade_marketplace_audit_log.actor_id FK → trade_marketplace_actors → violación
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- FIX: 4th param NULL (was auth.uid() → FK violation: auth user ≠ marketplace actor)
  ...
  -- NOTE: outbox 3rd param still auth.uid() here (fixed in migration 18)
