...
-- ok_rate = métrica de producto: presupuestos generados = OK_CATALOGO + OK_MIXTO + SOLO_SUGERIDAS
-- SOLO_SUGERIDAS es válido: el motor generó la estructura; el instalador añade precios desde
-- catálogos externos y el sistema aprende. No es un fallo del motor, es un gap del catálogo.
...
BEGIN
  SELECT
    -- ok = OK_CATALOGO + OK_MIXTO (con precio de catálogo)
  ...
  -- ok_rate = presupuestos generados (métrica de producto)
  -- Incluye SOLO_SUGERIDAS: el instalador añade precios → el catálogo aprende
  IF v_total > 0 THEN
    v_ok_rate := ROUND(
