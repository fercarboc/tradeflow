-- ═══════════════════════════════════════════════════════════════════════════
-- E4.C.3.3 — Migration 2: RPCs targeting estructurado
-- ═══════════════════════════════════════════════════════════════════════════
-- get_ad_targeting_options  — opciones validadas por tipo de objetivo
-- request_ad_slot_v2        — solicitud con targeting + snapshot económico
-- ensure_campaign_for_booking — actualizado con targeting estructurado
--
-- SOLICITUD ≠ RESERVA: request_ad_slot_v2 crea REQUESTED, no bloquea slot.
-- INVARIANTE: publicidad ≠ ranking. Sin subastas ni pricing dinámico.
-- ═══════════════════════════════════════════════════════════════════════════

-- ────────────────────────────────────────────────────────────────────────────
-- 1. get_ad_targeting_options
--    Devuelve opciones validadas para el tipo de objetivo pedido.
--    SECURITY DEFINER: lee tablas de taxonomía directamente.
-- ────────────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────────────
-- 2. request_ad_slot_v2
--    Crea booking REQUESTED con targeting validado server-side y snapshot
--    económico calculado en base a la tarifa del slot.
--    El total estimado es ORIENTATIVO — no constituye oferta ni factura.
-- ────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Estimación total (orientativa, calculada server-side)
  ...
  -- Validación de objetivo (si se proporciona)
...
-- ────────────────────────────────────────────────────────────────────────────
-- 3. ensure_campaign_for_booking — actualizado con targeting estructurado
--    Preserva compatibilidad con destination_type/destination_value existente.
-- ────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Si la campaña ya existe, devolverla
  ...
  -- Mapear targeting estructurado → destination_type/value (compat)
