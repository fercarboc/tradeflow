-- ════════════════════════════════════════════════════════════════════════════
-- MP-FIN-2B · Migración 13
-- Refunds + Partial Refunds — dominio completo de devoluciones económicas
-- ════════════════════════════════════════════════════════════════════════════
--
-- PRINCIPIO FUNDAMENTAL: REFUND = MOVIMIENTO COMPENSATORIO
--   La venta original NUNCA se modifica.
--   SALE +100 / REFUND -30 → ambas permanecen.
--   El refund pertenece siempre a un Supplier Order.
--   Sin contaminación entre proveedores.
--
-- MODELO (Phase 2B):
--   pending = SUM(GOODS_ENTITLEMENT + SHIPPING_ENTITLEMENT          ← positivos
--               + GOODS_REFUND_REVERSAL + SHIPPING_REFUND_REVERSAL)  ← negativos
--   Ledger entries para refunds tienen amount NEGATIVO (compensatorio).
--
-- ROUNDING:
--   Proporcional desde snapshots. El ÚLTIMO refund que cierra la línea absorbe
--   el remanente exacto para garantizar sum(refunds) = original exactamente.
--
-- SIMULATION ONLY:
--   simulation_only = true. STRIPE_GATE cerrado. Sin Stripe refunds.
--   REAL COMMISSION = 0 → COMMISSION_REVERSAL = 0.
--   La simulación 2% no afecta ningún refund real.
-- ════════════════════════════════════════════════════════════════════════════

BEGIN;

-- ── 1. Columnas de estado de refund en supplier orders ──────────────────────
...
-- ── 2. Columna de estado agregado en master orders ───────────────────────────
...
CREATE TABLE IF NOT EXISTS public.trade_marketplace_refunds (
  ...
  refund_number        text          NOT NULL,  -- RF-YYYY-NNNN
  ...
  -- Importes desde snapshots (nunca recalculados desde catálogo/config actual)
  ...
  -- COMPUTED: items_gross + shipping_gross
  ...
  -- Timeline (en simulation: requested_at = processed_at = now())
  ...
  CONSTRAINT chk_refund_simulation  CHECK (simulation_only = true)  -- STRIPE_GATE
);
...
-- ── 4. Tabla de líneas de refund ──────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS public.trade_marketplace_refund_items (
  id                        uuid          NOT NULL DEFAULT gen_random_uuid(),
  refund_id                 uuid          NOT NULL,
  order_item_id             uuid,         -- NULL para refund shipping o partial_amount sin linea

  -- De snapshots originales (nunca de catálogo actual)
  quantity_original         numeric(15,4),
  quantity_refunded         numeric(15,4),

  unit_price_gross_snapshot numeric(15,4),  -- precio_unitario (gross)
  unit_price_net_snapshot   numeric(15,4),  -- precio_unitario_neto_snapshot
  tax_rate_snapshot         numeric(7,4),   -- iva_porcentaje del snapshot
...
COMMENT ON TABLE public.trade_marketplace_refund_items IS
  'Líneas de devolución vinculadas a trade_marketplace_order_items. '
  'Los importes se calculan siempre desde snapshots (item_gross_snapshot, etc.), '
  'NUNCA desde precio actual del catálogo. '
  'order_item_id=NULL para shipping-only o partial_amount sin línea específica.';
...
-- Refund items: hereda del refund padre
...
-- ── 8. Helper: actualizar refund_status del supplier order ──────────────────────
...
-- ── 9. Helper: actualizar estado agregado del master order ──────────────────────
...
-- ── 10. mkt_fin_get_refundable_summary ───────────────────────────────────────
-- Devuelve lo que queda por devolver de un supplier_order.
-- Determinista: lee solo snapshots + refunds procesados.
...
BEGIN
  ...
  -- Totales ya devueltos (goods y shipping por separado)
  SELECT
...
-- ── 11. Helper interno: calcular importes de líneas para un refund ───────────
-- p_items: [{order_item_id, quantity_refunded}]
-- Retorna: {items_gross, items_net, items_tax, line_details}

CREATE OR REPLACE FUNCTION public._mkt_calc_item_refund_amounts(
  p_supplier_order_id uuid,
  p_items             jsonb   -- [{order_item_id, quantity_refunded}]
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_item          jsonb;
...
BEGIN
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    -- Cargar order_item con sus snapshots
    ...
    -- Calcular ya devuelto para este item
    ...
    IF v_item->>'quantity_refunded' IS NULL THEN
      -- full_item: devolver todo lo restante
    ...
    -- Calcular importes con rounding correcto
    v_is_last := (v_qty_refunded >= v_remaining_qty);

    IF v_is_last THEN
      -- Último refund de la línea: absorber el remanente exacto (evita céntimos imposibles)
      v_gross := v_oi.line_gross - v_already_refunded_gross;
      v_net   := v_oi.line_net  - v_already_refunded_net;
    ELSE
      -- Proporcional desde snapshot unitario
...
-- ── 12. mkt_fin_preview_refund ───────────────────────────────────────────────
-- Dry-run: calcula lo que resultaría. NO escribe nada.
...
BEGIN
  ...
  -- Balance antes/después (desde proyección actual)
...
-- ── 13. mkt_fin_create_refund ─────────────────────────────────────────────────
-- Función principal. Atómica: entidad + líneas + ledger + balance + estados.
-- DETENTE si cualquier parte falla — ROLLBACK total.
...
BEGIN
  ...
  ELSIF p_refund_type IN ('full_item','partial_quantity') THEN
    ...
    v_items_net   := (v_items_calc->>'items_net')::numeric;
    v_items_tax   := (v_items_calc->>'items_tax')::numeric;
    -- Shipping adicional si se especifica
    IF p_shipping_refund_gross > 0 THEN
  ...
  IF p_refund_type IN ('full_item','partial_quantity') OR
    ...
    FOR v_line IN SELECT * FROM jsonb_array_elements(v_items_calc->'line_details') LOOP
      INSERT INTO public.trade_marketplace_refund_items (
        ...
        NULL,  -- tax_rate la inferimos del snapshot del item
  ...
  ELSIF p_refund_type = 'full_order' THEN
    -- Una fila por cada item restante del order
    INSERT INTO public.trade_marketplace_refund_items (
      refund_id, order_item_id,
      quantity_original, quantity_refunded,
    ...
    SELECT v_refund_id, oi.id,
      ...
      -- net: snapshot_net - ya devuelto net (rounding absorber)
  ...
  ELSIF p_refund_type = 'partial_amount' THEN
    -- Refund por importe sin línea específica
    INSERT INTO public.trade_marketplace_refund_items (
...
-- ── 15. mkt_fin_list_supplier_refunds ────────────────────────────────────────
...
-- ── 16. mkt_fin_admin_refunds_overview ───────────────────────────────────────
...
-- ── 17. Actualizar mkt_fin_rebuild_provider_balance ─────────────────────────
-- Phase 2B: pending incluye GOODS_REFUND_REVERSAL + SHIPPING_REFUND_REVERSAL (negativos)
...
BEGIN
  ...
  -- Phase 2B: pending = SUM(entitlements + reversals)
  -- GOODS_ENTITLEMENT/SHIPPING_ENTITLEMENT: positivos
  -- GOODS_REFUND_REVERSAL/SHIPPING_REFUND_REVERSAL: negativos (compensatorios)
  -- COMMISSION_ACCRUAL/COMMISSION_SIM_ACCRUAL: EXCLUIDOS (INV-B02)
  SELECT
...
-- ── 18. Actualizar mkt_fin_reconcile_provider_balance ────────────────────────
...
BEGIN
  ...
  -- Phase 2B: incluye GOODS/SHIPPING_REFUND_REVERSAL (negativos)
  SELECT
    COALESCE(SUM(amount) FILTER (
      WHERE entry_type IN (
        'GOODS_ENTITLEMENT', 'SHIPPING_ENTITLEMENT',
...
-- ── 19. Extender mkt_fin_admin_overview con refund totals ────────────────────
