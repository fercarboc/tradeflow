...
-- A3: añadir search_aliases a UPs
ALTER TABLE public.trade_marketplace_universal_products
  ADD COLUMN IF NOT EXISTS search_aliases text[] DEFAULT '{}';

-- A3: actualizar create_cart_from_quote
-- Mejoras:
--   1. Path 3 nuevo: alias match via search_aliases (confianza 0.75)
--   2. Path 4: nombre ILIKE conservado (confianza 0.55, solo 15 chars)
--   3. Eliminar familia-fallback que genera falsos positivos
...
BEGIN
  ...
  LOOP
    ...
    -- PATH 0: UP ya almacenado en el quote_item (fase futura — por ahora siempre NULL)
    ...
    -- PATH 3: alias match via search_aliases (fuerte, confianza 0.75)
    -- Solo usa aliases específicos — no aliases débiles de una sola palabra
    IF v_up_id IS NULL THEN
      ...
      WHERE up.validation_state = 'validated'
        ...
        AND EXISTS (
          ...
          WHERE length(alias_term) >= 8   -- solo aliases de ≥8 chars (evita falsas matches con alias cortos)
            AND lower(v_item.descripcion) LIKE '%' || lower(alias_term) || '%'
        )
      ORDER BY (
        -- preferir el alias más largo que coincide (más específico)
    ...
    -- PATH 4: nombre_canonico ILIKE (primer truncado, confianza baja)
    -- Eliminado el fallback por familia para evitar falsos positivos
    IF v_up_id IS NULL THEN
