...
CREATE TABLE IF NOT EXISTS public.trade_marketplace_disputes (
  ...
  chargeback_fee_amount  numeric(15,4) NOT NULL DEFAULT 0,
...
CREATE POLICY dispute_evidence_provider_select ON public.trade_marketplace_dispute_evidence
  ...
  USING (
    dispute_id IN (
      ...
      WHERE d.provider_actor_id = d.provider_actor_id
    )
  );
...
-- ── 8. FUNCION: mkt_fin_get_dispute_exposure ─────────────────────────────────
...
-- ── 9. FUNCION: mkt_fin_preview_dispute_outcome ──────────────────────────────
...
BEGIN
  ...
  IF NOT public._mkt_validate_dispute_transition(v_d.status, p_outcome) THEN
    RAISE EXCEPTION 'INVALID_TRANSITION: % to %', v_d.status, p_outcome;
  END IF;
...
-- ── 10. FUNCION: mkt_fin_create_dispute ──────────────────────────────────────
...
-- ── 11. FUNCION: mkt_fin_add_dispute_evidence ────────────────────────────────
...
-- ── 12. FUNCION: mkt_fin_simulate_dispute_outcome ────────────────────────────
...
-- ── 13. FUNCION: mkt_fin_get_dispute ─────────────────────────────────────────
...
-- ── 14. FUNCION: mkt_fin_list_supplier_disputes ──────────────────────────────
...
-- ── 15. FUNCION: mkt_fin_admin_disputes_overview ─────────────────────────────
