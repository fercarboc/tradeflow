...
-- RC1-C.6 BLOCK G: evitar ghost carts
-- Si ya existe un cart activo/reviewing para la misma quote+org, reutilizarlo
-- en lugar de crear uno nuevo. Previene carros huérfanos cuando el usuario
-- vuelve al presupuesto y pulsa "Comprar materiales" de nuevo.
...
BEGIN
  ...
  -- Reutilizar carrito activo existente para evitar ghost carts
  ...
  LOOP
    ...
    THEN
      ...
      ORDER BY
        CASE o.match_state WHEN 'matched' THEN 0 ELSE 1 END
      LIMIT 1;
    END IF;
