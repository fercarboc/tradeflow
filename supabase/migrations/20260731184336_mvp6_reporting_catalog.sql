...
BEGIN
  ...
  SELECT COUNT(*)::integer INTO v_sin_stock_count
  FROM public.trade_marketplace_supplier_offerings
  WHERE supplier_catalog_id = v_catalog_id AND activa = true AND stock_disponible = false;

  SELECT COUNT(*)::integer INTO v_sin_imagen_count
  FROM public.trade_marketplace_supplier_offerings
  WHERE supplier_catalog_id = v_catalog_id AND activa = true AND image_url IS NULL;

  SELECT COUNT(*)::integer INTO v_inactivos_count
  FROM public.trade_marketplace_supplier_offerings
  WHERE supplier_catalog_id = v_catalog_id AND activa = false;

  SELECT COALESCE(jsonb_object_agg(match_state, cnt), '{}'::jsonb) INTO v_match_distribution
  FROM (
  ...
  SELECT ROUND(AVG(match_confidence)::numeric, 3) INTO v_avg_confidence
  FROM public.trade_marketplace_supplier_offerings
  WHERE supplier_catalog_id = v_catalog_id AND match_confidence IS NOT NULL;

  SELECT COUNT(*)::integer INTO v_pending_review_count
  FROM public.trade_marketplace_supplier_offerings
  WHERE supplier_catalog_id = v_catalog_id AND match_state = 'pending_review';
