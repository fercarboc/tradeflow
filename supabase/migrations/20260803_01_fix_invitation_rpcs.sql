...
-- Problema: gen_random_bytes está en 'extensions', no en 'public'
-- Solución: usar extensions.gen_random_bytes(32) explícitamente
-- ============================================================

-- ── Fix 1: create_marketplace_invitation ────────────────────
...
BEGIN
  ...
  FROM public.trade_marketplace_actors a WHERE a.id = p_actor_id;

  SELECT r.actor_type INTO v_role_type
  FROM public.trade_marketplace_roles r WHERE r.id = p_role_id;
  ...
  IF EXISTS (
    ...
    WHERE id = p_role_id AND nombre = 'platform_super_admin'
  ) THEN
  ...
  VALUES (p_actor_id, auth.uid(), 'invitation_created',
    jsonb_build_object(
      'role_id',           p_role_id,
      'email',             v_email_clean,
      'token_hash_prefix', left(v_token_hash, 8)
    )
  );
...
-- ── Fix 2: resend_supplier_invitation ───────────────────────
...
-- ── Nueva función: preview_marketplace_invitation ───────────
-- Sin autenticación requerida — devuelve solo datos públicamente seguros.
-- No expone token_hash ni datos internos.
-- Permite al frontend mostrar el proveedor/rol/email antes de que el usuario
-- inicie sesión o cree cuenta.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('estado', 'invalid_token');
  END IF;

  -- Comprobar expiración sin modificar el estado (read-only preview)
  IF v_inv.estado = 'accepted' THEN
    RETURN jsonb_build_object('estado', 'already_accepted', 'email', v_inv.email);
  END IF;

  IF v_inv.estado = 'revoked' THEN
    RETURN jsonb_build_object('estado', 'revoked', 'email', v_inv.email);
  END IF;

  IF v_inv.estado != 'pending' THEN
    RETURN jsonb_build_object('estado', v_inv.estado, 'email', v_inv.email);
  END IF;

  IF v_inv.expires_at < now() THEN
    RETURN jsonb_build_object('estado', 'expired', 'email', v_inv.email, 'expires_at', v_inv.expires_at);
  END IF;

  -- Invitación válida — devolver info pública
  RETURN jsonb_build_object(
    'estado',       'valid',
    'actor_id',     v_inv.actor_id,
    'actor_nombre', v_inv.actor_nombre,
    'role_nombre',  v_inv.role_nombre,
    'email',        v_inv.email,
...
'Devuelve información pública de una invitación por token raw. No requiere auth. Solo datos seguros.';
