-- FIX: mkt_fin_outbox_publish recibe actor_id con FK → trade_marketplace_actors.
-- auth.uid() es el auth user (cf1000d3), no un marketplace actor.
-- Fix: pasar v_order.actor_id (provider) — mismo patrón que mkt_fin_create_refund.
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    SELECT id INTO v_dispute_id FROM public.trade_marketplace_disputes WHERE idempotency_key = p_idempotency_key;
    IF FOUND THEN
      RETURN (SELECT jsonb_build_object('status','replayed','dispute_id',id,'dispute_number',dispute_number,'supplier_order_id',supplier_order_id,'amount',amount,'dispute_status',status,'responsibility',responsibility) FROM public.trade_marketplace_disputes WHERE id = v_dispute_id);
    END IF;
  END IF;

  -- Idempotencia por source_event_id
  IF p_source_event_id IS NOT NULL THEN
    SELECT id INTO v_dispute_id FROM public.trade_marketplace_disputes WHERE source_event_id = p_source_event_id;
    IF FOUND THEN
      RETURN (SELECT jsonb_build_object('status','replayed','dispute_id',id,'dispute_number',dispute_number,'supplier_order_id',supplier_order_id,'amount',amount,'dispute_status',status,'responsibility',responsibility) FROM public.trade_marketplace_disputes WHERE id = v_dispute_id);
    END IF;
  END IF;

  -- Cargar snapshot (FIX: actor_id, no provider_actor_id)
  SELECT o.id, o.master_order_id, o.actor_id,
    ...
    INTO v_order FROM public.trade_marketplace_orders o WHERE o.id = p_supplier_order_id;

  IF NOT FOUND THEN RAISE EXCEPTION 'ORDER_NOT_FOUND: %', p_supplier_order_id; END IF;
  IF v_order.total_gross = 0 THEN RAISE EXCEPTION 'ORDER_NO_SNAPSHOT: supplier order sin amounts snapshot'; END IF;
  ...
  END IF;
  IF p_amount <= 0 THEN RAISE EXCEPTION 'AMOUNT_MUST_BE_POSITIVE: %', p_amount; END IF;
  ...
  INSERT INTO public.trade_marketplace_disputes (
    ...
    v_dispute_num, v_order.master_order_id, p_supplier_order_id,
    v_order.actor_id, NULL,
    v_order.currency::char(3), p_amount, p_reason, p_reason_code,
    p_responsibility, true,
    p_evidence_due_at, p_external_dispute_id, p_external_payment_id,
    p_source_event_id, p_idempotency_key, v_corr, p_metadata, auth.uid()
  )
  RETURNING id INTO v_dispute_id;

  -- FIX: NULL como p_actor_id (no FK a marketplace_actors)
  ...
  -- FIX: v_order.actor_id (marketplace actor), no auth.uid() (auth user)
  PERFORM public.mkt_fin_outbox_publish(
    ...
    v_order.actor_id, NULL  -- FIX: provider actor, no auth.uid()
  );
