-- ============================================================
-- VF-GAP-LEDGER-IMMUTABILITY: fix fail-closed en fn_emitir_factura
-- Reemplaza auth.uid() IS NULL por auth.role() como discriminador.
-- ============================================================
--
-- Comportamiento objetivo:
--   authenticated  → auth.uid() obligatorio + check owner/member
--   service_role   → permitido (contexto admin autorizado, JWT explícito)
--   auth.role()=NULL → permitido (ejecución SQL directa, postgres/migrations)
--   anon           → RECHAZADO (no debería llegar: sin EXECUTE grant)
--   cualquier otro → RECHAZADO
--
-- Defensa en profundidad: también revocamos EXECUTE de anon
-- aunque actualmente no tiene el grant.
-- ============================================================
...
BEGIN
  ...
  -- auth.role() lee de request.jwt.claims — fiable en Supabase PostgREST.
  -- Es el discriminador correcto; auth.uid() IS NULL no distingue
  -- service_role de anon de ejecución SQL directa.
  v_jwt_role := auth.role();

  IF v_jwt_role = 'authenticated' THEN
    -- Sesión de usuario normal: uid obligatorio + pertenencia a org.
  ...
  ELSIF v_jwt_role = 'service_role' OR v_jwt_role IS NULL THEN
    -- service_role (JWT con role=service_role) o ejecución SQL directa
    -- (postgres/migrations — request.jwt.claims vacío → auth.role()=NULL).
    -- Ambos son contextos de confianza explícita a nivel de infraestructura.
    -- No se hace check de org: el caller es responsable de p_org_id correcto.
    NULL;

  ELSE
    -- 'anon' o cualquier otro rol no reconocido → siempre rechazado.
    RAISE EXCEPTION
  ...
  -- WHERE org_id = p_org_id garantiza que el invoice_id pertenece
  -- a la org autorizada; un invoice de otra org devuelve NOT FOUND.
  SELECT * INTO v_inv
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Factura no encontrada (id: %). Comprueba que pertenece a tu organización.', p_invoice_id
      USING ERRCODE = 'P0002';
  END IF;
