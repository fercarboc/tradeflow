...
-- En el portal del proveedor, 'delivered' (instalador confirmó recepción)
-- debe mostrarse como 'completed' para que aparezca en el tab "Completados"
...
BEGIN
  ...
  WITH all_orders AS (
    ...
    -- Pedidos marketplace: 'delivered' → 'completed' (instalador confirmó recepción = ciclo cerrado)
    SELECT
      mo.id                  AS r_id,
...
-- Verificar OBRAMAT Demo como actor activo y verificado para el piloto
UPDATE public.trade_marketplace_actors
SET verificado = true
WHERE slug = 'obramat-demo';
;
