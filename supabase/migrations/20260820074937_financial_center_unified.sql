...
SET pricing_mode = 'validation_free',
...
-- ── 6. Backfill invoices existentes ──────────────────────────────────────
DO $$
DECLARE r record;
BEGIN
...
-- ── 7. Backfill reservas publicitarias confirmadas ────────────────────────
INSERT INTO public.trade_financial_documents (
  doc_number, doc_series, revenue_type, payer_type, document_type,
  estado, payment_status,
  actor_id, customer_name,
  concept, period_start, period_end,
  rate_amount, subtotal, discount_amount, promotion_amount, commercial_value,
...
-- ── 8. RPC: admin_get_financial_documents ─────────────────────────────────
...
RETURNS TABLE (
  id uuid, doc_number text, doc_series text, revenue_type text, payer_type text,
  document_type text, estado text, payment_status text,
  org_id uuid, actor_id uuid, customer_name text, customer_nif text, customer_email text,
  concept text, period_start date, period_end date,
  rate_amount numeric, discount_amount numeric, promotion_amount numeric,
  commercial_value numeric, net_amount numeric, tax_rate numeric, tax_amount numeric,
  total_amount numeric, currency text,
  paid_at timestamptz, payment_method text, stripe_invoice_id text,
  invoice_url text, invoice_pdf_url text, issued_at timestamptz, sent_at timestamptz,
  metadata jsonb, created_at timestamptz,
  org_nombre text, actor_nombre text, slot_nombre text
)
...
BEGIN
  RETURN QUERY
  SELECT
    d.id, d.doc_number, d.doc_series, d.revenue_type, d.payer_type,
    d.document_type, d.estado, d.payment_status,
    d.org_id, d.actor_id, d.customer_name, d.customer_nif, d.customer_email,
    d.concept, d.period_start, d.period_end,
    d.rate_amount, d.discount_amount, d.promotion_amount,
    d.commercial_value, d.net_amount, d.tax_rate, d.tax_amount,
    d.total_amount, d.currency::text,
    d.paid_at, d.payment_method, d.stripe_invoice_id,
    d.invoice_url, d.invoice_pdf_url, d.issued_at, d.sent_at,
...
-- ── 9. RPC: admin_get_financial_summary ──────────────────────────────────
...
BEGIN
  RETURN jsonb_build_object(
    'kpis', jsonb_build_object(
      'facturado_mes',
        COALESCE((SELECT SUM(commercial_value) FROM public.trade_financial_documents
          WHERE to_char(COALESCE(period_start, created_at::date), 'YYYY-MM') = v_month), 0),
      'cobrado_mes',
        COALESCE((SELECT SUM(total_amount) FROM public.trade_financial_documents
          WHERE payment_status = 'paid' AND to_char(paid_at, 'YYYY-MM') = v_month), 0),
      'pendiente',
        COALESCE((SELECT SUM(total_amount) FROM public.trade_financial_documents
          WHERE payment_status = 'unpaid' AND estado NOT IN ('draft','cancelled')), 0),
      'ytd_cobrado',
        COALESCE((SELECT SUM(total_amount) FROM public.trade_financial_documents
          WHERE payment_status = 'paid' AND to_char(COALESCE(paid_at, created_at), 'YYYY') = v_year), 0),
      'valor_comercial_bonificado',
        COALESCE((SELECT SUM(commercial_value) FROM public.trade_financial_documents
          WHERE payment_status = 'waived'), 0),
      'total_docs', (SELECT COUNT(*) FROM public.trade_financial_documents),
      'ticket_medio',
        COALESCE((SELECT AVG(total_amount) FROM public.trade_financial_documents
          WHERE total_amount > 0 AND payment_status = 'paid'), 0)
    ),
    'por_tipo', (
      ...
      FROM (
        ...
        GROUP BY revenue_type ORDER BY valor_comercial DESC
      ) r
    ),
    'ad_espacios', (
      SELECT jsonb_agg(row_to_json(r))
      FROM (
        SELECT
          s.id AS slot_id, s.nombre AS slot_nombre, s.formato,
    ...
    'saas_planes', (
      SELECT jsonb_agg(row_to_json(r))
      FROM (
        SELECT ts.plan, ts.billing_cycle,
        ...
        GROUP BY ts.plan, ts.billing_cycle ORDER BY ts.plan, ts.billing_cycle
      ) r
    )
  );
END;
...
-- ── 10. RPC: admin_create_ad_financial_document ──────────────────────────
...
AS $$
DECLARE
  v_booking record; v_slot record; v_actor record;
  v_doc_id uuid; v_doc_num text;
  v_rate numeric; v_disc numeric; v_promo numeric; v_net numeric;
BEGIN
  ...
  IF p_pricing_mode = 'validation_free' THEN
    v_promo := v_rate - v_disc; v_net := 0;
  ELSE
    v_promo := 0; v_net := v_rate - v_disc;
  END IF;
  ...
  IF v_doc_id IS NOT NULL THEN
    UPDATE public.trade_financial_documents SET
      rate_amount = v_rate, discount_amount = v_disc, promotion_amount = v_promo,
      commercial_value = v_rate, net_amount = v_net, total_amount = v_net,
  ...
  INSERT INTO public.trade_financial_documents (
    doc_number, doc_series, revenue_type, payer_type, document_type, estado, payment_status,
    actor_id, customer_name, concept, period_start, period_end,
    rate_amount, discount_amount, promotion_amount, commercial_value,
    net_amount, total_amount, currency, ad_booking_id, metadata
  ) VALUES (
    ...
    v_booking.actor_id, COALESCE(v_actor.nombre, 'Proveedor'),
    ...
    v_rate, v_disc, v_promo, v_rate, v_net, v_net, 'EUR',
    p_booking_id,
    jsonb_build_object(
      'slot_id', v_booking.slot_id, 'slot_nombre', v_slot.nombre,
      'pricing_mode', p_pricing_mode, 'discount_rate', p_discount_rate,
