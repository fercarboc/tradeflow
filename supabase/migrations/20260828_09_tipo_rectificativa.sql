...
-- Campo tipo_rectificativa (I/S) según Orden HAC/1177/2024.
--
-- La especificación distingue TipoFactura (R1-R4, Lista L2)
-- de TipoRectificativa (I=diferencias, S=sustitución):
--   son campos semánticos distintos en el RegistroAlta.
--
-- MVP TrabFlow: solo soporta I (por diferencias).
--   - trade_invoices.tipo_rectificativa TEXT ('I'|'S'|NULL)
--   - trade_fiscal_records.tipo_rectificativa TEXT ('I'|'S'|NULL)
--   - fn_crear_factura_rectificativa: hardcodea 'I'
--   - fn_emitir_factura v7: valida 'I', rechaza NULL y 'S'
--   - fn_protect_emitted_invoice: protege el campo post-emisión
--
-- HASH: TipoRectificativa NO entra en el hash canónico AEAT v0.1.2.
--   El hash sigue usando exclusivamente los 8 campos oficiales.
-- ============================================================

-- ── 1. Columna en trade_invoices ─────────────────────────────
...
-- ── 2. Columna en trade_fiscal_records ───────────────────────
-- El ledger debe poder reconstruir el RegistroAlta de forma autónoma
-- sin depender de datos mutables de trade_invoices.
...
-- ── 3. Actualizar fn_protect_emitted_invoice ─────────────────
-- Añade tipo_rectificativa a la lista de campos protegidos.
...
-- El usuario solo elige causa/motivo.
-- TrabFlow fija tipo_rectificativa='I' (único modelo MVP soportado).
-- El RPC NO acepta p_tipo_rectificativa como parámetro:
-- evitar que el frontend pueda introducir 'S' por error o manipulación.
...
BEGIN
  ...
  -- Valores admitidos: R1-R4 (R5 bloqueado — solo para rectificativas de F2).
  IF p_tipo_factura_vf NOT IN ('R1', 'R2', 'R3', 'R4') THEN
    RAISE EXCEPTION
      'tipo_factura_vf no válido para rectificativa de factura completa: "%". '
      'Valores admitidos: R1 (error/art.80.1-2-6), R2 (art.80.3 concurso), '
      'R3 (art.80.4 incobrable), R4 (resto).',
  ...
  INSERT INTO public.trade_invoices (
    ...
    tipo_factura,
    tipo_factura_vf,
    tipo_rectificativa,      -- siempre 'I' en MVP
    serie,
    estado,
    numero,
    concepto,
    ...
    'rectificativa',
    p_tipo_factura_vf,
    'I',                     -- único modelo soportado en TrabFlow MVP
    'R',
    'Borrador',
    v_numero_tmp,
...
--   - Valida tipo_rectificativa='I' para rectificativas (rechaza NULL y 'S')
--   - Persiste tipo_rectificativa en trade_fiscal_records
--   - Hash sin cambios (TipoRectificativa NO entra en el canonical hash)
...
DECLARE
  ...
  v_hash            text;
  v_tipo_vf         text;
  v_tipo_rect       text;   -- 'I', NULL (no existe 'S' en MVP)
...
BEGIN
  ...
  -- ── 3. Determinar TipoFactura y TipoRectificativa ────────────
  IF v_inv.tipo_factura = 'rectificativa' THEN
    ...
    IF v_inv.rectifica_factura_id IS NULL THEN
      RAISE EXCEPTION
        'La factura rectificativa no tiene referencia a la factura original.'
        USING ERRCODE = 'P0001';
    END IF;
    ...
    -- Validar tipo_rectificativa = 'I' (único modelo soportado en MVP).
    -- Rechazar NULL (campo no inicializado) y 'S' (por sustitución, no implementado).
    IF v_inv.tipo_rectificativa IS NULL THEN
      RAISE EXCEPTION
        'La rectificativa no tiene tipo_rectificativa definido. '
        'Debe valer ''I'' (por diferencias). Recrear mediante fn_crear_factura_rectificativa.'
        USING ERRCODE = 'P0001';
    END IF;
    IF v_inv.tipo_rectificativa != 'I' THEN
      RAISE EXCEPTION
        'TipoRectificativa "%" no soportado en este MVP. '
        'Solo se admite rectificación por diferencias (I).',
    ...
    v_tipo_vf   := v_inv.tipo_factura_vf;   -- R1, R2, R3 o R4
    v_tipo_rect := 'I';                      -- siempre 'I' en MVP
  ...
  -- ── 10. Hash input — formato oficial AEAT v0.1.2 ─────────────
  -- TipoRectificativa NO entra en el hash canónico.
  -- El hash solo usa los 8 campos oficiales.
  v_hash_input :=
  ...
  -- ── 12. Insertar en ledger fiscal ─────────────────────────────
  -- tipo_rectificativa se persiste en el ledger para que el RegistroAlta
  -- pueda reconstruirse de forma autónoma (sin depender de trade_invoices).
  -- NULL para facturas ordinarias F1.
  ...
  -- ── 13. Actualizar factura a Emitida ──────────────────────────
  UPDATE public.trade_invoices SET
    ...
    -- tipo_rectificativa NO se actualiza aquí: ya está correctamente
    -- establecido por fn_crear_factura_rectificativa ('I') o es NULL
    -- para ordinarias. El trigger lo protege post-emisión.
