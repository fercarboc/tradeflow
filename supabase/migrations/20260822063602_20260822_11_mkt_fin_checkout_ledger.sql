...
--
-- COMISION SIMULADA: NO se escribe en el ledger.
--   Calculada via simulation_rate en reporting.
--   Objetivo: evitar ambiguedad entre comision real (0) e hipotesis analitica.
--
-- IDEMPOTENCIA:
...
BEGIN
  ...
  LOOP
    ...
    -- 7b. SHIPPING_ENTITLEMENT (solo si > 0)
    ...
    -- 7c. COMMISSION: NO se escribe (commission=0 en Phase 0)
  ...
  IF v_suppliers_count = 0 THEN
    RAISE EXCEPTION
      ...
      'financial_snapshot_at. Verificar que checkout_cart_v2 completo correctamente.',
      p_master_order_id;
  END IF;
...
COMMENT ON FUNCTION public.mkt_fin_post_checkout_ledger IS
  'Genera las entradas iniciales del ledger de simulacion para un checkout completo. '
...
BEGIN
  ...
  IF v_cart.estado NOT IN ('active', 'reviewing') THEN
    RAISE EXCEPTION 'CART_NOT_READY: El carrito no esta en estado activo (estado: %)', v_cart.estado;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'NO_ITEMS: No hay items con proveedor seleccionado.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'ACTOR_INACTIVE: Uno o mas proveedores seleccionados ya no estan activos.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'OFFERING_INACTIVE: Una o mas offerings seleccionadas ya no estan disponibles.';
  END IF;
  ...
  -- STEP 7: Post simulation ledger (MP-FIN-1B.2) — no-blocking
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    PERFORM public.mkt_fin_audit(
      ...
      'Ledger posting fallo tras checkout exitoso — reintentable',
...
COMMENT ON FUNCTION public.checkout_cart_v2 IS
  'Checkout atomico: crea supplier_orders, snapshots financieros, master_order '
