-- RC1-C.7.A — Portal Pedidos: filtro de tienda + columnas delivery_method / pickup_location_id / tienda_nombre
--
-- Cambios:
--   1. DROP de la firma anterior (4 params IN) — PostgreSQL no permite OR REPLACE al cambiar firma.
--   2. CREATE nueva firma con p_location_id uuid DEFAULT NULL (5.º parámetro).
--   3. Nuevas columnas de salida: delivery_method, pickup_location_id, tienda_nombre.
--   4. Comportamiento:
--      - p_location_id IS NULL  → igual que antes (legacy + marketplace).
--      - p_location_id IS NOT NULL → solo marketplace con pickup_location_id = p_location_id.
--        Los pedidos legacy no tienen dimensión tienda y se excluyen automáticamente.
--   5. total_count refleja siempre el conjunto filtrado (window function OVER () tras WHERE).
--   6. tienda_nombre = COALESCE(pickup_location_snapshot->>'nombre', sl.nombre)
--      Prioridad snapshot histórico; fallback a location viva si snapshot falta.
--   7. Seguridad: _mkt_has_permission(p_actor_id, 'orders:read') igual que antes.
--   8. RLS: marketplace filtrado por actor_id = p_actor_id; legacy por catalog_id = v_catalog_id.

-- ── 1. Drop firma antigua ─────────────────────────────────────────────────────
...
BEGIN
  ...
  WITH all_orders AS (
    ...
    -- Pedidos legacy: solo cuando no hay filtro de tienda (no tienen dimensión location)
    SELECT
      so.id                  AS r_id,
    ...
    WHERE so.catalog_id = v_catalog_id
      ...
      AND p_location_id IS NULL   -- excluir pedidos legacy cuando hay filtro por tienda
    ...
    SELECT
      ...
      -- tienda_nombre: prioridad snapshot histórico; fallback a location viva
      COALESCE(
