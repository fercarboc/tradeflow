...
CREATE TABLE IF NOT EXISTS public.trade_marketplace_actors (
  ...
  actor_type          text        NOT NULL REFERENCES public.trade_marketplace_actor_types(id) ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.trade_marketplace_actor_members (
  ...
  role_id           uuid        NOT NULL REFERENCES public.trade_marketplace_roles(id) ON DELETE RESTRICT,
  activo            boolean     NOT NULL DEFAULT true,
...
CREATE TABLE IF NOT EXISTS public.trade_marketplace_member_permission_overrides (
  id          uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  member_id   uuid        NOT NULL REFERENCES public.trade_marketplace_actor_members(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_actor_sup_cat   ON public.trade_marketplace_actors(supplier_catalog_id) WHERE supplier_catalog_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_role_type       ON public.trade_marketplace_roles(actor_type);
CREATE INDEX IF NOT EXISTS idx_role_actor      ON public.trade_marketplace_roles(actor_id) WHERE actor_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_member_user_activo ON public.trade_marketplace_actor_members(user_id, activo) WHERE activo = true;
...
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_caller_id uuid; v_caller_priority integer; v_new_priority integer; v_new_role_nombre text;
BEGIN
  IF TG_OP = 'UPDATE' AND NEW.role_id = OLD.role_id THEN RETURN NEW; END IF;
...
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_event text; v_data jsonb; v_uid uuid; v_aid uuid;
BEGIN
  v_uid := auth.uid();
  IF TG_OP = 'INSERT' THEN
    v_aid := NEW.actor_id; v_event := CASE WHEN NEW.accepted_at IS NULL THEN 'member_invited' ELSE 'member_joined' END;
    v_data := jsonb_build_object('member_id', NEW.id, 'user_id', NEW.user_id, 'role_id', NEW.role_id);
  ELSIF TG_OP = 'UPDATE' THEN
    v_aid := NEW.actor_id;
    IF OLD.role_id IS DISTINCT FROM NEW.role_id THEN
      v_event := 'member_role_changed'; v_data := jsonb_build_object('member_id', NEW.id, 'user_id', NEW.user_id, 'old_role_id', OLD.role_id, 'new_role_id', NEW.role_id);
  ...
  ELSIF TG_OP = 'DELETE' THEN
    v_aid := OLD.actor_id; v_event := 'member_removed';
...
BEGIN
  IF OLD.estado IS DISTINCT FROM NEW.estado THEN
    INSERT INTO public.trade_marketplace_audit_log (actor_id, user_id, event_type, event_data)
    VALUES (NEW.id, auth.uid(), 'actor_estado_changed', jsonb_build_object('old_estado', OLD.estado, 'new_estado', NEW.estado));
  END IF;
  IF OLD.verificado IS DISTINCT FROM NEW.verificado AND NEW.verificado = true THEN
    INSERT INTO public.trade_marketplace_audit_log (actor_id, user_id, event_type, event_data)
    VALUES (NEW.id, auth.uid(), 'actor_verified', jsonb_build_object('verificado_by', NEW.verificado_by));
  END IF;
  RETURN NEW;
END; $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_actor_updated_at' AND tgrelid = 'public.trade_marketplace_actors'::regclass) THEN
    CREATE TRIGGER trg_actor_updated_at BEFORE UPDATE ON public.trade_marketplace_actors FOR EACH ROW EXECUTE FUNCTION public.trg_set_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_a_member_type_compat' AND tgrelid = 'public.trade_marketplace_actor_members'::regclass) THEN
    CREATE TRIGGER trg_a_member_type_compat BEFORE INSERT OR UPDATE ON public.trade_marketplace_actor_members FOR EACH ROW EXECUTE FUNCTION public.trg_fn_mkt_member_type_compat();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_b_member_privilege' AND tgrelid = 'public.trade_marketplace_actor_members'::regclass) THEN
    CREATE TRIGGER trg_b_member_privilege BEFORE INSERT OR UPDATE ON public.trade_marketplace_actor_members FOR EACH ROW EXECUTE FUNCTION public.trg_fn_mkt_member_privilege_check();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_c_member_last_owner' AND tgrelid = 'public.trade_marketplace_actor_members'::regclass) THEN
    CREATE TRIGGER trg_c_member_last_owner BEFORE UPDATE OR DELETE ON public.trade_marketplace_actor_members FOR EACH ROW EXECUTE FUNCTION public.trg_fn_mkt_member_last_owner();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_member_audit' AND tgrelid = 'public.trade_marketplace_actor_members'::regclass) THEN
    CREATE TRIGGER trg_member_audit AFTER INSERT OR UPDATE OR DELETE ON public.trade_marketplace_actor_members FOR EACH ROW EXECUTE FUNCTION public.trg_fn_mkt_member_audit();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_actor_audit' AND tgrelid = 'public.trade_marketplace_actors'::regclass) THEN
    CREATE TRIGGER trg_actor_audit AFTER UPDATE ON public.trade_marketplace_actors FOR EACH ROW EXECUTE FUNCTION public.trg_fn_mkt_actor_audit();
  END IF;
END; $$;

-- Helper functions definidas ANTES de las políticas RLS
-- Devuelve uuid[] (array) para ser usable en expresiones de política
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.trade_marketplace_actor_members m JOIN public.trade_marketplace_roles r ON r.id = m.role_id
    WHERE m.actor_id = p_actor_id AND m.user_id = auth.uid() AND m.activo = true AND r.permissions @> jsonb_build_array(p_permission)
  );
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.trade_marketplace_actor_members m JOIN public.trade_marketplace_actors a ON a.id = m.actor_id
    JOIN public.trade_marketplace_roles r ON r.id = m.role_id
    WHERE m.user_id = auth.uid() AND m.activo = true AND a.actor_type = 'platform' AND a.estado = 'active'
...
RETURNS integer LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT r.priority FROM public.trade_marketplace_actor_members m JOIN public.trade_marketplace_roles r ON r.id = m.role_id
  WHERE m.actor_id = p_actor_id AND m.user_id = auth.uid() AND m.activo = true LIMIT 1;
$$;
...
CREATE POLICY "actor_types_select" ON public.trade_marketplace_actor_types FOR SELECT USING (activo = true);
...
CREATE POLICY "actors_select_member"   ON public.trade_marketplace_actors FOR SELECT TO authenticated USING (id = ANY(public._mkt_actor_ids_for_user()));
...
CREATE POLICY "actors_update"          ON public.trade_marketplace_actors FOR UPDATE TO authenticated USING (public._mkt_has_permission(id, 'actor:settings') OR public._mkt_is_platform_admin());
...
CREATE POLICY "roles_select_system" ON public.trade_marketplace_roles FOR SELECT TO authenticated USING (is_system = true AND actor_id IS NULL);
CREATE POLICY "roles_select_actor"  ON public.trade_marketplace_roles FOR SELECT TO authenticated USING (actor_id = ANY(public._mkt_actor_ids_for_user()));
...
CREATE POLICY "members_select_own" ON public.trade_marketplace_actor_members FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY "members_select"     ON public.trade_marketplace_actor_members FOR SELECT TO authenticated
  USING (actor_id = ANY(public._mkt_actor_ids_for_user()) AND public._mkt_has_permission(actor_id, 'members:read'));
CREATE POLICY "members_update"     ON public.trade_marketplace_actor_members FOR UPDATE TO authenticated
  USING (actor_id = ANY(public._mkt_actor_ids_for_user()) AND public._mkt_has_permission(actor_id, 'members:manage'));
...
CREATE POLICY "invitations_select" ON public.trade_marketplace_invitations FOR SELECT TO authenticated
  USING (actor_id = ANY(public._mkt_actor_ids_for_user()) AND public._mkt_has_permission(actor_id, 'members:read'));
CREATE POLICY "invitations_insert" ON public.trade_marketplace_invitations FOR INSERT TO authenticated
  WITH CHECK (actor_id = ANY(public._mkt_actor_ids_for_user()) AND public._mkt_has_permission(actor_id, 'members:invite'));
CREATE POLICY "invitations_update" ON public.trade_marketplace_invitations FOR UPDATE TO authenticated
  USING (actor_id = ANY(public._mkt_actor_ids_for_user()) AND public._mkt_has_permission(actor_id, 'members:invite'));

DROP POLICY IF EXISTS "overrides_platform" ON public.trade_marketplace_member_permission_overrides;
CREATE POLICY "overrides_platform" ON public.trade_marketplace_member_permission_overrides FOR ALL TO authenticated USING (public._mkt_is_platform_admin());
...
RETURNS TABLE (actor_id uuid, actor_type text, actor_nombre text, actor_slug text, actor_estado text, actor_logo_url text, actor_verificado boolean, member_id uuid, role_id uuid, role_nombre text, role_priority integer, permissions jsonb, activo boolean, accepted_at timestamptz)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT a.id, a.actor_type, a.nombre, a.slug, a.estado, a.logo_url, a.verificado,
    m.id, m.role_id, r.nombre, r.priority, r.permissions, m.activo, m.accepted_at
  FROM public.trade_marketplace_actor_members m JOIN public.trade_marketplace_actors a ON a.id = m.actor_id JOIN public.trade_marketplace_roles r ON r.id = m.role_id
...
DECLARE v_inv RECORD; v_user_id uuid; v_email text; v_member_id uuid;
BEGIN
  v_user_id := auth.uid();
  ...
  SELECT * INTO v_inv FROM public.trade_marketplace_invitations WHERE token = p_token;
  ...
  END IF;
  IF lower(v_email) != lower(v_inv.email) THEN RAISE EXCEPTION 'EMAIL_MISMATCH: Esta invitación es para "%".', v_inv.email; END IF;
...
BEGIN
  ...
  IF p_new_owner_user_id = v_caller_id THEN RAISE EXCEPTION 'SELF_TRANSFER'; END IF;
  SELECT actor_type INTO v_actor_type FROM public.trade_marketplace_actors WHERE id = p_actor_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'ACTOR_NOT_FOUND: %', p_actor_id; END IF;
  ...
  IF NOT FOUND OR v_caller_member.role_nombre != 'owner' THEN RAISE EXCEPTION 'NOT_OWNER'; END IF;
...
('supplier','owner','Control total del proveedor','["offerings:read","offerings:write","offerings:delete","offerings:match","catalog:import","catalog:export","orders:read","orders:manage","orders:fulfillment","analytics:read","analytics:export","members:read","members:invite","members:manage","billing:read","billing:manage","promotions:write","actor:settings","actor:delete"]',true,100),
('supplier','admin','Gestión completa excepto billing:manage y actor:delete','["offerings:read","offerings:write","offerings:delete","offerings:match","catalog:import","catalog:export","orders:read","orders:manage","orders:fulfillment","analytics:read","analytics:export","members:read","members:invite","members:manage","billing:read","promotions:write","actor:settings"]',true,80),
('supplier','catalog_manager','Gestión de catálogo y matching de productos','["offerings:read","offerings:write","offerings:delete","offerings:match","catalog:import","catalog:export","analytics:read"]',true,60),
('supplier','orders_manager','Gestión y confirmación de pedidos recibidos','["orders:read","orders:manage","orders:fulfillment","analytics:read"]',true,60),
('supplier','finance','Acceso a facturación y métricas financieras','["billing:read","billing:manage","analytics:read","analytics:export","orders:read"]',true,50),
('supplier','member_manager','Gestión del equipo del proveedor','["members:read","members:invite","members:manage","analytics:read"]',true,50),
('supplier','analyst','Solo lectura de analytics y catálogo','["analytics:read","analytics:export","offerings:read","orders:read"]',true,30),
('supplier','viewer','Lectura mínima sin capacidad de escritura','["offerings:read","orders:read","analytics:read"]',true,10),
('brand','owner','Control total del fabricante','["offerings:read","offerings:write","offerings:delete","offerings:match","catalog:import","catalog:export","analytics:read","analytics:export","members:read","members:invite","members:manage","billing:read","billing:manage","promotions:write","actor:settings","actor:delete"]',true,100),
('brand','admin','Gestión completa excepto actor:delete','["offerings:read","offerings:write","offerings:delete","offerings:match","catalog:import","catalog:export","analytics:read","analytics:export","members:read","members:invite","members:manage","billing:read","promotions:write","actor:settings"]',true,80),
('brand','catalog_manager','Gestión del catálogo oficial y matching a UPs','["offerings:read","offerings:write","offerings:delete","offerings:match","catalog:import","catalog:export","analytics:read"]',true,60),
('brand','marketing','Gestión de promociones y visibilidad de marca','["promotions:write","analytics:read","analytics:export","offerings:read"]',true,40),
('brand','analyst','Métricas y rendimiento de catálogo','["analytics:read","analytics:export","offerings:read"]',true,30),
('brand','viewer','Solo lectura de catálogo y analytics','["offerings:read","analytics:read"]',true,10),
('logistics','owner','Control total del operador logístico','["orders:read","orders:manage","orders:fulfillment","logistics:manage","analytics:read","analytics:export","members:read","members:invite","members:manage","billing:read","billing:manage","actor:settings","actor:delete"]',true,100),
('logistics','admin','Gestión completa excepto actor:delete','["orders:read","orders:manage","orders:fulfillment","logistics:manage","analytics:read","analytics:export","members:read","members:invite","members:manage","billing:read","actor:settings"]',true,80),
('logistics','dispatcher','Asignación de rutas y gestión de entregas','["orders:read","orders:manage","orders:fulfillment","logistics:manage","analytics:read"]',true,60),
('logistics','analyst','Métricas logísticas solo lectura','["analytics:read","analytics:export","orders:read"]',true,30),
('logistics','viewer','Lectura mínima de pedidos','["orders:read","analytics:read"]',true,10),
('association','owner','Control total de la asociación','["members:read","members:invite","members:manage","analytics:read","analytics:export","billing:read","billing:manage","actor:settings","actor:delete"]',true,100),
('association','admin','Gestión completa excepto actor:delete','["members:read","members:invite","members:manage","analytics:read","analytics:export","billing:read","actor:settings"]',true,80),
('association','member_manager','Gestión del directorio de miembros','["members:read","members:invite","members:manage","analytics:read"]',true,50),
('association','analyst','Métricas de la asociación solo lectura','["analytics:read","analytics:export"]',true,30),
('association','viewer','Solo lectura','["analytics:read"]',true,10),
('platform','platform_super_admin','Super administrador de TrabFlow','["offerings:read","offerings:write","offerings:delete","offerings:match","catalog:import","catalog:export","orders:read","orders:manage","analytics:read","analytics:export","members:read","members:invite","members:manage","billing:read","billing:manage","promotions:write","logistics:manage","actor:settings","actor:delete","platform:actors","platform:verify","api:access"]',true,999),
('platform','admin','Administrador de TrabFlow','["platform:actors","platform:verify","analytics:read","analytics:export","members:read","orders:read","offerings:read","billing:read"]',true,80),
('platform','support','Soporte técnico y atención al cliente','["orders:read","members:read","offerings:read","analytics:read"]',true,40),
('platform','analyst','Analista de datos de la plataforma','["analytics:read","analytics:export"]',true,30),
('any','viewer','Lectura mínima universal (todos los tipos de actor)','["offerings:read","analytics:read"]',true,10)
ON CONFLICT ON CONSTRAINT uq_role_type_nombre_actor DO NOTHING;

INSERT INTO public.trade_marketplace_actors (actor_type, nombre, slug, estado, verificado)
VALUES ('platform', 'TrabFlow', 'trabflow-platform', 'active', true)
