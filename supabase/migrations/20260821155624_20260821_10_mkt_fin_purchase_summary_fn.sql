...
BEGIN
  ...
  ELSE
    ...
    IF NOT FOUND THEN
      -- Pedido legacy (sin master_order_id)
      SELECT * INTO v_order
  ...
  -- Supplier orders completos con items
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
...
COMMENT ON FUNCTION public.mkt_fin_get_purchase_summary_data IS
  ...
  'INV-007: inmutabilidad garantizada. INV-005: comisión no expuesta al comprador. '
