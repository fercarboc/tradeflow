...
-- Tiempos de ciclo, SLA, incidencias, pedidos urgentes y retrasados.
...
BEGIN
  ...
  -- ── Tiempos medios de ciclo (solo marketplace, solo cuando existen timestamps) ──
  -- Confirmación: created_at → confirmed_at
  ...
  -- Preparación: confirmed_at → preparing_at (si existe la fase de preparación)
  SELECT ROUND(AVG(EXTRACT(EPOCH FROM (preparing_at - confirmed_at)) / 3600.0)::numeric, 1)
  INTO v_avg_preparing_h
  FROM public.trade_marketplace_orders
  WHERE actor_id = p_actor_id
    AND preparing_at IS NOT NULL AND confirmed_at IS NOT NULL
    AND created_at >= p_date_from AND created_at < p_date_to;

  -- Envío desde confirmación: confirmed_at → shipped_at
  ...
  -- Ciclo total: created_at → delivered_at (solo pedidos entregados)
  ...
  -- ── SLA: % confirmados en < 24h ────────────────────────────────────────────
  SELECT COUNT(*) FILTER (WHERE confirmed_at IS NOT NULL)::integer
  INTO v_confirmed_orders
  FROM public.trade_marketplace_orders
  WHERE actor_id = p_actor_id
    AND estado NOT IN ('cancelled')
    AND created_at >= p_date_from AND created_at < p_date_to;

  SELECT COUNT(*) FILTER (WHERE shipped_at IS NOT NULL)::integer
  INTO v_shipped_orders
  FROM public.trade_marketplace_orders
  WHERE actor_id = p_actor_id
    AND estado NOT IN ('cancelled')
    AND created_at >= p_date_from AND created_at < p_date_to;
  ...
  -- ── SLA: % enviados en < 48h desde confirmación ────────────────────────────
  SELECT ROUND(
    COUNT(*) FILTER (
  ...
  -- ── Pedidos atrasados activos (>72h sin avanzar de confirmed/preparing) ─────
  SELECT COUNT(*)::integer
  INTO v_atrasados_count
  FROM public.trade_marketplace_orders
  WHERE actor_id = p_actor_id
    AND estado IN ('confirmed', 'preparing', 'pending')
    AND created_at < now() - INTERVAL '72 hours';

  -- ── Incidencias totales en período ────────────────────────────────────────
