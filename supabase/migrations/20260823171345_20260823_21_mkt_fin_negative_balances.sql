...
-- ══════════════════════════════════════════════════════════════════════════
-- PARTE 6 — mkt_fin_get_provider_balance Phase 2D
-- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT jsonb_agg(flag) INTO v_risk_flags
    FROM (
      SELECT jsonb_build_object('flag', 'AGING_CRITICAL', 'days', v_aging_days) AS flag
        WHERE v_aging_days > 90
      UNION ALL
      SELECT jsonb_build_object('flag', 'NO_RECOVERY_PLAN', 'days', v_aging_days)
        WHERE v_aging_days > 30 AND NOT v_bal.recovery_in_progress
      UNION ALL
      SELECT jsonb_build_object('flag', 'HIGH_DEFICIT', 'amount', v_bal.negative_amount)
        WHERE v_bal.negative_amount > 1000
    ) flags;
  ...
  RETURN jsonb_build_object(
    ...
    'risk_flags',           COALESCE(v_risk_flags, '[]'::jsonb),
...
-- ══════════════════════════════════════════════════════════════════════════
-- PARTE 8 — mkt_fin_preview_recovery
-- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_bal.negative_amount = 0 THEN
    RAISE EXCEPTION 'NO_DEFICIT: actor=% currency=% no tiene balance negativo', p_actor_id, p_currency;
  END IF;
