...
BEGIN
  ...
  WHERE actor_id = p_actor_id AND slot_id = p_slot_id
  ...
  ELSIF p_target_type IS NOT NULL AND p_target_id IS NOT NULL THEN
    ...
    ELSIF p_target_type = 'SUPPLIER' THEN
      IF p_target_id <> p_actor_id THEN RAISE EXCEPTION 'Un proveedor solo puede promocionarse a sí mismo como SUPPLIER'; END IF;
  ...
  INSERT INTO public.trade_marketplace_ad_bookings (
    ...
    estimated_days_snapshot, estimated_total_snapshot, commercial_terms_snapshot
  ) VALUES (
    p_slot_id, p_actor_id, 'REQUESTED', p_inicio, p_fin, 'portal_supplier', p_mensaje,
    ...
    jsonb_build_object('rate_amount',v_rate_amount,'rate_currency',COALESCE(v_rate_currency,'EUR'),
      'rate_unit',v_rate_unit,'days',v_days,'estimated_total',v_estimated_total,
      'target_type',p_target_type,'target_label',v_target_label,'target_ids',p_target_ids,'snapshot_at',now()::text)
  ) RETURNING id INTO v_booking_id;
  RETURN jsonb_build_object('request_id', v_booking_id);
END;
$$;

GRANT EXECUTE ON FUNCTION public.request_ad_slot_v2(uuid, text, date, date, text, text, uuid, uuid[]) TO authenticated;;
