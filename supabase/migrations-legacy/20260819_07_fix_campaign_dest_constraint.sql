...
-- ═══════════════════════════════════════════════════════════════════════════
-- Root cause: ensure_campaign_for_booking usaba lower(target_type) para
-- destination_type, pero chk_campaign_dest no incluía 'trade', 'brand' ni
-- 'offering'. Booking con TRADE → destination_type='trade' → constraint fail.
-- Fix 1: ampliar constraint para incluir los tipos de targeting estructurado.
-- Fix 2: mapeo explícito en la función (OFFERING→'offer', no 'offering').
-- ═══════════════════════════════════════════════════════════════════════════

-- Fix 1: ampliar chk_campaign_dest
...
-- Fix 2: ensure_campaign_for_booking con mapeo explícito
...
BEGIN
  ...
  -- Mapeo explícito target_type → destination_type (OFFERING→'offer', no 'offering')
