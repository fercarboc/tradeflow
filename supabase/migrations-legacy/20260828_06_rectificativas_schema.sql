...
-- Almacena el código fiscal VeriFactu (F1, R1-R5) por separado
-- de motivo_rectificacion (texto humano).
-- Nullable: se asigna en fn_crear_factura_rectificativa (Borrador R)
-- y en fn_emitir_factura (F1 para ordinarias al momento de emitir).
ALTER TABLE public.trade_invoices
  ADD COLUMN IF NOT EXISTS tipo_factura_vf text;

-- Constraint de valores válidos (Lista L2 Orden HAC/1177/2024).
-- No incluye F3 (facturas en sustitución de simplificadas) — fuera de scope.
ALTER TABLE public.trade_invoices
...
-- Incluye tipo_factura_vf.
-- Restaura iva_importe y total (presentes en _01, perdidos en _02).
...
AS $$
BEGIN
  -- Mientras sea Borrador, todo puede cambiar.
  ...
  -- Campos fiscales protegidos post-emisión.
  -- Allowlist mutable: estado, paid_at, metodo_pago, fecha_vencimiento,
  --                    notas_internas, devuelta_at, devuelta_motivo, updated_at.
