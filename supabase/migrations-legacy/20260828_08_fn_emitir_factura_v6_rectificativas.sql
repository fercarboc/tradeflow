-- ============================================================
-- VF-RECTIFICATIVAS-IMPL FASE 1 — Migration 08
-- fn_emitir_factura v6: desbloquea emisión de rectificativas.
--
-- Cambios respecto a v5 (_05):
--   1. Sustituye el RAISE EXCEPTION incondicional de rectificativas
--      por validación estricta con rama dedicada.
--   2. v_tipo_vf se lee de tipo_factura_vf (ya persistido en Borrador)
--      para rectificativas — NUNCA se infiere de serie.
--   3. Validación adicional de la factura original para rectificativas.
--   4. Al emitir (UPDATE final), persiste tipo_factura_vf='F1'
--      para facturas ordinarias, garantizando trazabilidad completa.
--   5. Añade v_orig record para verificar la original.
-- ============================================================
...
DECLARE
  ...
  v_inv             record;
  v_org             record;
  v_orig            record;   -- factura original (solo para rectificativas)
  v_prev            record;   -- último registro en la cadena fiscal
  v_year            int;
  v_count           int;
  v_numero          text;
...
BEGIN
  ...
  ELSIF v_jwt_role = 'service_role' OR v_jwt_role IS NULL THEN
    NULL; -- contextos de confianza (admin, migrations, tests)
  ...
  -- Para rectificativas: v_tipo_vf viene del campo persistido tipo_factura_vf.
  -- Para ordinarias: siempre F1 (TrabFlow no emite facturas simplificadas F2).
  -- NUNCA se infiere desde serie (serie R != TipoFactura R1).
  IF v_inv.tipo_factura = 'rectificativa' THEN
    ...
    -- Validar referencia a la original
    ...
    -- Verificar factura original: existe, misma org, no Borrador
  ...
  ELSE
    -- Facturas ordinarias: F1 (completa).
    -- F2 (simplificada) no se usa en TrabFlow.
    v_tipo_vf := 'F1';
  END IF;
  ...
  -- serie R usa el mismo algoritmo COUNT-based que F y M.
  -- El advisory lock garantiza que dos emisiones R concurrentes
  ...
  -- La serie R NO inicia una cadena separada: encadena con el
  -- último registro fiscal de la org (sea F, M o R).
  ...
  -- ── 10. Importes ──────────────────────────────────────────
  -- Para rectificativas los importes son negativos.
  -- COALESCE permite que funcionen aunque iva_importe/total no estén
  -- explícitamente calculados (fn_crear_factura_rectificativa sí los calcula).
  ...
  -- TipoFactura=R1 (o R2/R3/R4) para rectificativas.
  -- CuotaTotal e ImporteTotal son negativos para rectificativas
  -- por diferencias — correcto según la especificación.
  ...
  -- record_type = 'alta' para todas las facturas (incluidas rectificativas).
  -- Las anulaciones VeriFactu son un registro separado (VF-CANCELLATION).
  ...
  -- tipo_factura_vf se escribe aquí también para F1 ordinarias,
  -- garantizando trazabilidad completa en trade_invoices.
  -- Para rectificativas ya estaba escrito (no cambia).
