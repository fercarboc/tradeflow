-- ═══════════════════════════════════════════════════════════════════════════
-- E4.D — Migration 3: Fix get_active_campaigns con booking guards
-- ═══════════════════════════════════════════════════════════════════════════
-- PROBLEMA: Las campañas de proveedor se seguían mostrando incluso si la
-- reserva estaba CANCELLED/EXPIRED o el periodo había caducado.
--
-- FIX: Para campaign_source='supplier', se añade guardia de reserva:
--   · booking.estado IN ('RESERVED','CONFIRMED')
--   · booking.inicio <= CURRENT_DATE (periodo ya comenzó)
--   · booking.fin   >= CURRENT_DATE (periodo no ha terminado)
--
-- INVARIANTE: publicidad ≠ ranking. No se toca pricing ni score.
-- ═══════════════════════════════════════════════════════════════════════════
...
WITH ranked AS (
  ...
  WHERE c.slot_id = ANY(p_slot_ids)
    ...
    -- Guardia de reserva para campañas de proveedor
    -- Bloque 12 (caducidad) + Bloque 13 (cancelación): defensa en servidor
    AND (
