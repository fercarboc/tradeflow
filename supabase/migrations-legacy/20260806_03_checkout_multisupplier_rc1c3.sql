...
-- Fecha: 2026-08-06
-- Requiere: Sprint 1B, 1C, 1D y migraciones RC1-C.1/C.2
-- ═══════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.trade_marketplace_supplier_pickup_points (
  ...
  -- { calle, cp, ciudad, provincia, pais, horario, instrucciones }
  telefono      text,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 02. AMPLIAR trade_marketplace_supplier_config — métodos de pago permitidos
-- ─────────────────────────────────────────────────────────────────────────────
...
COMMENT ON COLUMN public.trade_marketplace_supplier_config.payment_methods_allowed IS
  'Métodos de pago que acepta este proveedor en el checkout. Valores: cuenta_proveedor, transferencia, pago_recoger, online, domiciliacion.';

-- ─────────────────────────────────────────────────────────────────────────────
-- 03. AMPLIAR trade_marketplace_orders — delivery, pago, comprador
-- ─────────────────────────────────────────────────────────────────────────────
ALTER TABLE public.trade_marketplace_orders
  ...
  -- { nombre, empresa, nif, email, telefono, numero_cliente }
  ...
  -- checkout_key: idempotency key generado por el cliente (UUID4), guardado por referencia

COMMENT ON COLUMN public.trade_marketplace_orders.cart_id_v2 IS
  'FK explícita al carrito. Distinto de cart_id (Sprint 1D) para no romper schema.';
COMMENT ON COLUMN public.trade_marketplace_orders.buyer_snapshot IS
  'Snapshot del comprador en el momento del pedido. Inmutable.';
COMMENT ON COLUMN public.trade_marketplace_orders.checkout_key IS
  'Clave de idempotencia generada por el cliente. Usada para detectar doble envío.';
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 05. RPC — CHECKOUT V2 (multiproveedor, con entrega, pago, comprador, idempotencia)
-- ─────────────────────────────────────────────────────────────────────────────
-- p_delivery_data JSONB: { "actor_uuid": { delivery_method, delivery_address JSONB,
--   pickup_point_id, payment_method, notas } }
-- p_buyer_snapshot JSONB: { nombre, empresa, nif, email, telefono }
-- p_checkout_key TEXT: UUID generado por el cliente (idempotencia)
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ── Revalidación de proveedores y offerings ───────────────────────────────
  -- Verificar que todos los actores seleccionados siguen activos
  IF EXISTS (
    SELECT 1
  ...
  -- Verificar que las offerings siguen activas
  IF EXISTS (
    SELECT 1
    FROM public.trade_marketplace_cart_items ci
  ...
  FOR v_actor IN
    ...
    WHERE ci.cart_id = p_cart_id AND ci.activo = true AND ci.selected_actor_id IS NOT NULL
  LOOP
    -- Calcular subtotal del pedido para este proveedor
    ...
    -- Calcular portes según config del proveedor
    v_portes := 0;
    ...
    -- Extraer datos de entrega para este actor
    ...
    -- Insertar líneas del pedido (snapshot inmutable)
    ...
    -- Outbox para notificaciones externas
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 06. RPC — ESTADO DE PEDIDOS POR IDS (para pantalla de éxito)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 07. RLS en trade_marketplace_supplier_pickup_points (supplier lectura/escritura)
-- ─────────────────────────────────────────────────────────────────────────────
-- Ya creado arriba (sección 01).
