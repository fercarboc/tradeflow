...
-- Tablas: trade_supplier_api_credentials, trade_supplier_api_sync_log,
--         trade_supplier_api_idempotency, trade_supplier_api_rate_limits
-- Funciones: create_api_credential, revoke_api_credential, rotate_api_credential,
--            get_api_credentials, resolve_api_credential, check_and_increment_rate_limit,
--            get_idempotency_record, set_idempotency_record,
--            log_api_sync_start, log_api_sync_end, get_supplier_api_sync_log,
--            cleanup_api_expired_records
-- ═══════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.trade_supplier_api_credentials (
  ...
  key_prefix      text        NOT NULL,        -- primeros 8 chars del hex raw (solo display)
  key_hash        text        NOT NULL UNIQUE, -- SHA-256(raw_key) en hex; nunca el secreto raw
  scopes          text[]      NOT NULL,
  activa          boolean     NOT NULL DEFAULT true,
  expires_at      timestamptz NOT NULL,        -- obligatorio; sin excepción
  grace_until     timestamptz,                 -- set durante rotación; antigua válida hasta aquí
...
CREATE INDEX IF NOT EXISTS idx_api_cred_actor
  ON public.trade_supplier_api_credentials(actor_id);
CREATE INDEX IF NOT EXISTS idx_api_cred_key_hash
  ON public.trade_supplier_api_credentials(key_hash);
CREATE INDEX IF NOT EXISTS idx_api_cred_activa
  ON public.trade_supplier_api_credentials(actor_id, activa, expires_at);
...
CREATE TABLE IF NOT EXISTS public.trade_supplier_api_sync_log (
  ...
  error_detail    text,       -- nunca contiene el Bearer token

  CONSTRAINT chk_sync_status CHECK (
    status IN ('processing','completed','failed','duplicate')
  )
);

CREATE INDEX IF NOT EXISTS idx_api_sync_actor
  ON public.trade_supplier_api_sync_log(actor_id, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_api_sync_cred
  ON public.trade_supplier_api_sync_log(credential_id, started_at DESC);
...
CREATE POLICY "api_sync_no_insert"
  ON public.trade_supplier_api_sync_log FOR INSERT TO authenticated
  WITH CHECK (false);

-- ─────────────────────────────────────────────────────────────────────────────
-- 03. STORE DE IDEMPOTENCIA (TTL 24h)
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.trade_supplier_api_idempotency (
  ...
  actor_id         uuid        NOT NULL
    REFERENCES public.trade_marketplace_actors(id) ON DELETE CASCADE,
  endpoint         text        NOT NULL,
  request_hash     text        NOT NULL,   -- SHA-256(body) para detectar IDEMPOTENCY_CONFLICT
...
CREATE INDEX IF NOT EXISTS idx_api_idem_expires
  ON public.trade_supplier_api_idempotency(expires_at);

-- Sin RLS: gestionado exclusivamente por SECURITY DEFINER vía service role
...
CREATE POLICY "api_idem_deny_all"
  ON public.trade_supplier_api_idempotency FOR ALL TO authenticated
  USING (false);

-- ─────────────────────────────────────────────────────────────────────────────
-- 04. CONTADORES DE RATE LIMIT (ventana deslizante por minuto)
-- ─────────────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_api_rl_window
  ON public.trade_supplier_api_rate_limits(window_start);
...
CREATE POLICY "api_rl_deny_all"
  ON public.trade_supplier_api_rate_limits FOR ALL TO authenticated
  USING (false);

-- ═══════════════════════════════════════════════════════════════════════════════
-- FUNCIONES DE GESTIÓN (llamadas desde el Portal con sesión autenticada)
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF trim(p_nombre) = '' OR p_nombre IS NULL THEN
    RAISE EXCEPTION 'INVALID_INPUT: nombre es obligatorio.'
      USING ERRCODE = 'invalid_parameter_value';
  END IF;

  IF p_expires_at IS NULL OR p_expires_at <= now() THEN
    RAISE EXCEPTION 'INVALID_INPUT: expires_at debe ser una fecha futura.'
      USING ERRCODE = 'invalid_parameter_value';
  END IF;

  IF p_scopes IS NULL OR array_length(p_scopes, 1) < 1 THEN
    RAISE EXCEPTION 'INVALID_INPUT: Se requiere al menos un scope.'
      USING ERRCODE = 'invalid_parameter_value';
  END IF;

  IF NOT (p_scopes <@ ARRAY['catalog:read','catalog:write','stock:write','prices:write','imports:read']) THEN
    RAISE EXCEPTION 'INVALID_INPUT: Scope no válido. Permitidos: catalog:read, catalog:write, stock:write, prices:write, imports:read.'
      USING ERRCODE = 'invalid_parameter_value';
  END IF;

  -- Generar: raw = 'tsf_v1_' + 64 hex chars (32 bytes aleatorios)
  ...
  INSERT INTO public.trade_supplier_api_credentials (
    actor_id, nombre, key_prefix, key_hash, scopes, expires_at, created_by
  ) VALUES (
    p_actor_id, trim(p_nombre), v_key_prefix, v_key_hash, p_scopes, p_expires_at, auth.uid()
  ) RETURNING id INTO v_cred_id;

  -- raw_key se devuelve UNA SOLA VEZ; nunca vuelve a estar disponible
...
BEGIN
  ...
  FROM public.trade_supplier_api_credentials
  WHERE id = p_credential_id;

  IF NOT FOUND OR v_owner <> p_actor_id THEN
    RAISE EXCEPTION 'NOT_FOUND: Credencial no encontrada o sin acceso.'
      USING ERRCODE = 'no_data_found';
  END IF;

  UPDATE public.trade_supplier_api_credentials
  SET activa = false, revoked_at = now(), revoked_by = auth.uid(),
      grace_until = NULL
  WHERE id = p_credential_id;
END;
$$;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND: Credencial no encontrada, sin acceso o ya revocada.'
      USING ERRCODE = 'no_data_found';
  END IF;
  ...
  INSERT INTO public.trade_supplier_api_credentials (
    ...
    v_old.actor_id,
    v_old.nombre || ' (rotada ' || to_char(now(), 'DD/MM') || ')',
    v_key_prefix, v_key_hash,
    v_old.scopes,
    GREATEST(v_old.expires_at, now() + interval '1 year'),
    auth.uid()
  ) RETURNING id INTO v_new_id;

  -- Antigua entra en período de gracia 24h; después cleanup la desactiva
  UPDATE public.trade_supplier_api_credentials
  SET grace_until = v_grace
  WHERE id = p_credential_id;

  RETURN jsonb_build_object(
    'credential_id',     v_new_id,
    'raw_key',           v_raw_key,
    'key_prefix',        v_key_prefix,
    'old_credential_id', p_credential_id,
    'grace_until',       v_grace
  );
END;
$$;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- get_api_credentials — NUNCA devuelve key_hash
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- FUNCIONES DE INFRAESTRUCTURA (solo llamables con service_role desde Edge Functions)
-- ═══════════════════════════════════════════════════════════════════════════════

-- ─────────────────────────────────────────────────────────────────────────────
-- resolve_api_credential — valida Bearer hash, actualiza last_used
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.resolve_api_credential(
  p_key_hash text,
...
DECLARE
  v_cred  RECORD;
BEGIN
  SELECT c.id, c.actor_id, c.scopes, c.nombre,
         c.activa, c.expires_at, c.grace_until,
  ...
  -- Credencial inválida si: no activa Y fuera de gracia
  IF NOT v_cred.activa
     AND (v_cred.grace_until IS NULL OR v_cred.grace_until < now())
  ...
  UPDATE public.trade_supplier_api_credentials
  SET last_used_at = now(), last_ip = p_ip, last_user_agent = p_ua
  WHERE id = v_cred.id;

  RETURN jsonb_build_object(
    'credential_id', v_cred.id,
    'actor_id',      v_cred.actor_id,
    'scopes',        v_cred.scopes,
    'nombre',        v_cred.nombre
  );
END;
$$;
...
DECLARE
  ...
  v_count    integer;
BEGIN
  INSERT INTO public.trade_supplier_api_rate_limits
    (actor_id, endpoint, window_start, request_count)
  VALUES
    (p_actor_id, p_endpoint, v_window, 1)
...
-- get_idempotency_record / set_idempotency_record
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.get_idempotency_record(
  p_key      text,
  p_actor_id uuid,
  p_endpoint text
)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'request_hash',    v_rec.request_hash,
...
AS $$
BEGIN
  INSERT INTO public.trade_supplier_api_idempotency
    (idempotency_key, actor_id, endpoint, request_hash, response_status, response_body)
  VALUES
    (p_key, p_actor_id, p_endpoint, p_request_hash, p_response_status, p_response_body)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- log_api_sync_start / log_api_sync_end
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.log_api_sync_start(
  ...
  p_rows_received   integer DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path = public
AS $$
...
BEGIN
  INSERT INTO public.trade_supplier_api_sync_log
    (credential_id, actor_id, endpoint, idempotency_key,
     source_system, ip, user_agent, rows_received)
  VALUES
    (p_credential_id, p_actor_id, p_endpoint, p_idempotency_key,
     p_source_system, p_ip, p_user_agent, p_rows_received)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- get_supplier_api_sync_log — para el Portal (sesión autenticada)
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.get_supplier_api_sync_log(
  p_actor_id uuid,
  ...
  RETURN COALESCE(
    (SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'rows_rejected',    l.rows_rejected,
        'error_detail',     l.error_detail,
        'credential_nombre', c.nombre,
        'credential_prefix', c.key_prefix
...
-- ─────────────────────────────────────────────────────────────────────────────
-- cleanup_api_expired_records — llamar con pg_cron diariamente
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Desactivar credenciales cuyo período de gracia ha expirado
  UPDATE public.trade_supplier_api_credentials
