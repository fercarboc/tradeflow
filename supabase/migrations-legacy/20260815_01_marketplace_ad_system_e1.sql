-- ============================================================
-- RC1-C.8.E — E1: Sistema publicitario Marketplace
-- Migración aplicada: 2026-08-15
-- Spec: v3.1.1 (hardening pre-ejecución)
--
-- INVARIANTE: publicidad ≠ ranking.
-- Esta migración NO toca catálogo, comparador, precios ni Universal Products.
-- trade_marketplace_promotions es un sistema diferente — no mezclar.
--
-- Rollback: ver sección final.
-- ============================================================

-- ── 1. Tabla: trade_marketplace_ad_slots ─────────────────────
-- Ubicaciones publicitarias. Registros insertados una sola vez;
-- cambiar = nueva versión frontend.
...
-- FK circular (fallback_campaign_id) se añade DESPUÉS de crear ad_campaigns.
...
CREATE TABLE IF NOT EXISTS public.trade_marketplace_ad_campaigns (
  ...
  oficio           text,       -- para matching fallback nivel 3
...
CREATE INDEX IF NOT EXISTS idx_ad_campaigns_actor
...
CREATE INDEX IF NOT EXISTS idx_ad_bookings_slot_estado
...
-- ── 4. Trigger anti-solapamiento con advisory lock (v3.1.1) ──
-- Problema original: SELECT EXISTS dentro de BEFORE TRIGGER es TOCTOU:
-- dos transacciones concurrentes pasan el check simultáneamente antes de
-- que ninguna haya commitado. Ambas insertan. Race condition.
--
-- Solución: pg_advisory_xact_lock(namespace=1000001, key=hashtext(slot_id))
-- serializa todas las operaciones de booking para el mismo slot.
-- Tx1 adquiere lock → Tx2 espera → Tx1 commit → Tx2 adquiere lock →
-- EXISTS encuentra el booking de Tx1 → RAISE EXCEPTION → rollback Tx2.
-- Sin extensiones adicionales. Lock liberado automáticamente en commit/rollback.
  ...
  IF NEW.estado IN ('CONFIRMED', 'RESERVED') THEN
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION
        ...
        USING ERRCODE = 'exclusion_violation';  -- SQLSTATE 23P01
    END IF;
  END IF;
  RETURN NEW;
END;
...
-- FK circular slots → fallback_campaign (resuelve ciclo slots ↔ campaigns)
...
-- Slots: lectura pública (catálogo de espacios). Sin INSERT directa.
CREATE POLICY ad_slots_select_public
  ON public.trade_marketplace_ad_slots
  FOR SELECT USING (true);

-- Campaigns: activas visibles por todos (renderizado instaladores);
--   proveedor ve las suyas; admin ve todo. Sin INSERT directa.
...
-- Bookings: proveedor ve los suyos; admin ve todo. Sin INSERT directa.
...
-- Creatives: misma regla que campaigns.
...
-- INVARIANTE: solo lee slots publicitarios.
-- Nunca toca catálogo, comparador, precios ni Universal Products.
-- Tiers (1 < 2 < 3 < 4): commercial → slot_fallback → trabflow_oficio → trabflow_global
-- priority solo desempata DENTRO del mismo tier.
-- Un trabflow con priority=999 NUNCA gana a un commercial con priority=1.
CREATE OR REPLACE FUNCTION public.get_active_campaigns(p_slot_ids text[])
RETURNS TABLE (
  slot_id           text,
  campaign_id       uuid,
  campaign_source   text,
  resolution_level  int,     -- 1=commercial | 2=slot_fallback | 3=trabflow_oficio | 4=trabflow_global
...
SELECT DISTINCT ON (slot_id)
  ...
  mobile_image_url
FROM ranked
ORDER BY slot_id, tier ASC, priority DESC, id  -- id = desempate determinista
$$;

CREATE INDEX IF NOT EXISTS idx_ad_campaigns_source_oficio
  ON public.trade_marketplace_ad_campaigns(campaign_source, oficio)
  WHERE activa = true AND estado = 'ACTIVE';

-- ── 8a. Seeds — 16 slots comerciales ─────────────────────────
...
-- ── 8b. Seeds — campañas fallback TrabFlow (1 por slot) ──────
-- activa=true, estado='ACTIVE', sin start_at/end_at = siempre activas.
-- campaign_source='trabflow', priority=0 (siempre bajo cualquier commercial).
...
VALUES
  ...
  ('MARKET_HOME_LEFT_TOP',      'trabflow','ACTIVE','TW-FB-LT', 'TrabFlow',
   'Electricidad profesional',         'Material eléctrico para instaladores',
   'Ver electricidad', 'category','electricidad', 'electricidad', 0,true,
   '#F59E0B','linear-gradient(160deg,#1c1917 0%,#292524 80%,#1c1917 100%)','#ffffff'),
  ('MARKET_HOME_LEFT_MID',      'trabflow','ACTIVE','TW-FB-LM', 'TrabFlow',
   'Fontanería y saneamiento',          'Tubería, valvulería y accesorios',
   'Ver fontanería',  'category','fontaneria',    'fontaneria',    0,true,
  ...
  ('MARKET_HOME_LEFT_BOTTOM',   'trabflow','ACTIVE','TW-FB-LB', 'TrabFlow',
   'Climatización y HVAC',              'Bombas de calor y aire acondicionado',
   'Ver climatización','category','climatizacion', 'climatizacion', 0,true,
  ...
  ('MARKET_HOME_RIGHT_TOP',     'trabflow','ACTIVE','TW-FB-RT', 'TrabFlow',
   'Materiales de fontanería',          'Stock inmediato · Precio profesional',
   'Ver fontanería',  'category','fontaneria',    'fontaneria',    0,true,
  ...
  ('MARKET_HOME_RIGHT_MID',     'trabflow','ACTIVE','TW-FB-RM', 'TrabFlow',
   'Carpintería y madera',              'Tableros, molduras y tarima',
   'Ver carpintería', 'category','carpinteria',   'carpinteria',   0,true,
  ...
  ('MARKET_HOME_RIGHT_BOTTOM',  'trabflow','ACTIVE','TW-FB-RB', 'TrabFlow',
   'Pinturas profesionales',            'Pinturas, imprimaciones y esmaltes',
   'Ver pintura',     'category','pintura',       'pintura',       0,true,
   '#c084fc','linear-gradient(160deg,#2e1065 0%,#4c1d95 80%,#2e1065 100%)','#ffffff'),
  -- Hero slides
  ('MARKET_HOME_HERO_1',        'trabflow','ACTIVE','TW-FB-H1', 'TrabFlow',
   'Instalación eléctrica de calidad',  'Material profesional desde catálogo',
   'Ver electricidad','category','electricidad',  'electricidad',  0,true,
   null,'linear-gradient(135deg,#78350f 0%,#d97706 100%)','#ffffff'),
  ('MARKET_HOME_HERO_2',        'trabflow','ACTIVE','TW-FB-H2', 'TrabFlow',
   'Fontanería desde catálogo',         'Todo en un solo lugar',
   'Ver fontanería',  'category','fontaneria',    'fontaneria',    0,true,
   null,'linear-gradient(135deg,#1e40af 0%,#3b82f6 100%)','#ffffff'),
  ('MARKET_HOME_HERO_3',        'trabflow','ACTIVE','TW-FB-H3', 'TrabFlow',
   'Todo tu material en un lugar',      'Compara proveedores y haz tus pedidos',
   'Explorar catálogo','catalog',null,             null,            0,true,
   null,'linear-gradient(135deg,#134e4a 0%,#14b8a6 100%)','#ffffff'),
  -- Mobile promos
  ('MARKET_HOME_MOBILE_PROMO_1','trabflow','ACTIVE','TW-FB-MP1','TrabFlow',
   'Material eléctrico disponible',     'Consulta disponibilidad en tiempo real',
   'Ver electricidad','category','electricidad',  'electricidad',  0,true,
   '#F59E0B','linear-gradient(120deg,#1c1917 0%,#292524 100%)','#ffffff'),
  ('MARKET_HOME_MOBILE_PROMO_2','trabflow','ACTIVE','TW-FB-MP2','TrabFlow',
   'Fontanería en catálogo',            'Pedidos desde el móvil',
   'Ver fontanería',  'category','fontaneria',    'fontaneria',    0,true,
   '#60a5fa','linear-gradient(120deg,#0f2044 0%,#1e3a5f 100%)','#ffffff'),
  -- PromoCards
  ('MARKET_HOME_PROMO_CARD_1',  'trabflow','ACTIVE','TW-FB-PC1','TrabFlow',
   'Electricidad profesional',          null,
   'Ver electricidad','category','electricidad',  'electricidad',  0,true,
   '#F59E0B','linear-gradient(135deg,#1c1917 0%,#292524 100%)','#ffffff'),
  ('MARKET_HOME_PROMO_CARD_2',  'trabflow','ACTIVE','TW-FB-PC2','TrabFlow',
   'Fontanería y saneamiento',          null,
   'Ver fontanería',  'category','fontaneria',    'fontaneria',    0,true,
   '#22d3ee','linear-gradient(135deg,#082f49 0%,#0c4a6e 100%)','#ffffff'),
  ('MARKET_HOME_PROMO_CARD_3',  'trabflow','ACTIVE','TW-FB-PC3','TrabFlow',
   'Climatización HVAC',                null,
   'Ver climatización','category','climatizacion', 'climatizacion', 0,true,
   '#14b8a6','linear-gradient(135deg,#0f3430 0%,#134e4a 100%)','#ffffff'),
  ('MARKET_HOME_PROMO_CARD_4',  'trabflow','ACTIVE','TW-FB-PC4','TrabFlow',
   'Más materiales',                    null,
   'Ver catálogo',    'catalog', null,             null,            0,true,
   '#60a5fa','linear-gradient(135deg,#0f2044 0%,#1e3a5f 100%)','#ffffff'),
  -- Catalog banner
  ('MARKET_CATALOG_HERO',       'trabflow','ACTIVE','TW-FB-CH', 'TrabFlow',
   'Compara precios entre distribuidores','Encuentra el mejor precio en tu zona',
   'Explorar catálogo','catalog',null,             null,            0,true,
   '#60a5fa','linear-gradient(90deg,#0f2044 0%,#1e3a5f 100%)','#ffffff')
ON CONFLICT DO NOTHING;

-- ── 8c. Wiring fallback_campaign_id ──────────────────────────
-- Conecta cada slot con su campaña TrabFlow para resolución tier=2 en la RPC.
-- Ejecutado después del INSERT de campañas, en la misma transacción.
...
-- ── ROLLBACK (ejecutar en orden inverso si se necesita deshacer E1) ───
-- DROP TRIGGER IF EXISTS tg_booking_no_overlap ON public.trade_marketplace_ad_bookings;
-- DROP FUNCTION IF EXISTS public._check_booking_no_overlap();
-- DROP FUNCTION IF EXISTS public.get_active_campaigns(text[]);
-- DROP TABLE IF EXISTS public.trade_marketplace_ad_creatives  CASCADE;
-- DROP TABLE IF EXISTS public.trade_marketplace_ad_campaigns  CASCADE;
-- DROP TABLE IF EXISTS public.trade_marketplace_ad_bookings   CASCADE;
-- DROP TABLE IF EXISTS public.trade_marketplace_ad_slots      CASCADE;
-- El frontend no se toca en E1 — rollback de BD no afecta la UI.
