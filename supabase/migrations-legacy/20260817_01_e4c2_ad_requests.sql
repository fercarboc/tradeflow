...
-- Decisión: Extender trade_marketplace_ad_bookings con estados CONTACTING/ACCEPTED/REJECTED
-- y columna mensaje. Supplier INSERT RLS. 3 RPCs de solicitud y gestión admin.
-- Anti-overlap trigger (_check_booking_no_overlap) ya solo verifica CONFIRMED/RESERVED.
-- CONTACTING/ACCEPTED/REJECTED no bloquean disponibilidad.
-- INVARIANTE: Publicidad ≠ Ranking. Estos estados nunca afectan ranking, precio ni recomendación.

-- ─── 1. Extender CHECK constraint de estado ───────────────────────────────────
...
-- ─── 2. Columna mensaje (proveedor → TrabFlow, distinta de notas admin) ──────

ALTER TABLE public.trade_marketplace_ad_bookings
  ADD COLUMN IF NOT EXISTS mensaje text;

-- ─── 3. INSERT RLS para proveedores ──────────────────────────────────────────
-- Solo REQUESTED + portal_request. Validación de negocio está en la RPC.
...
-- ─── 4. RPC: request_ad_slot ─────────────────────────────────────────────────
-- Crea solicitud REQUESTED. No bloquea disponibilidad.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = p_slot_id AND activo AND comercializable
  ) THEN
    RAISE EXCEPTION 'Slot % no existe o no está disponible', p_slot_id
      USING ERRCODE = 'foreign_key_violation';
  END IF;

  IF p_fin < p_inicio THEN
    RAISE EXCEPTION 'Fecha fin debe ser igual o posterior a inicio'
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Ya existe una solicitud pendiente para este espacio en ese período'
      USING ERRCODE = 'unique_violation';
  END IF;

  INSERT INTO public.trade_marketplace_ad_bookings
    (slot_id, actor_id, estado, inicio, fin, origen, mensaje)
  VALUES
    (p_slot_id, p_actor_id, 'REQUESTED', p_inicio, p_fin, 'portal_request', p_mensaje)
...
-- ─── 5. RPC: admin_update_ad_request_status ──────────────────────────────────
-- Solo admin. Transiciones válidas: REQUESTED/CONTACTING/ACCEPTED → CONTACTING/ACCEPTED/REJECTED/CANCELLED.
...
BEGIN
  ...
  IF v_current NOT IN ('REQUESTED', 'CONTACTING', 'ACCEPTED') THEN
    RAISE EXCEPTION 'No se puede cambiar estado desde %', v_current
      USING ERRCODE = 'check_violation';
  END IF;

  UPDATE public.trade_marketplace_ad_bookings
  SET estado       = p_status,
      notas        = COALESCE(p_admin_notes, notas),
...
-- ─── 6. RPC: admin_convert_request_to_booking ────────────────────────────────
-- Solo admin. ACCEPTED → RESERVED. Trigger verifica anti-overlap al cambiar a RESERVED.
...
BEGIN
  ...
  IF v_req.estado != 'ACCEPTED' THEN
    RAISE EXCEPTION 'Solo se pueden convertir solicitudes ACCEPTED, actual: %', v_req.estado
      USING ERRCODE = 'check_violation';
  END IF;
  ...
  -- Trigger _check_booking_no_overlap verifica conflictos RESERVED/CONFIRMED
  UPDATE public.trade_marketplace_ad_bookings
  SET estado       = 'RESERVED',
      inicio       = v_inicio,
      fin          = v_fin,
...
-- ─── 7. Actualizar admin_get_ads_dashboard con KPI solicitudes ───────────────
...
SELECT jsonb_build_object(
  ...
  'reservas_pendientes',            b.reservas_pendientes,
...
BEGIN
  ...
  IF v_cnt < 3 THEN
    RAISE EXCEPTION 'E4C2: solo % de 3 RPCs creadas', v_cnt;
  END IF;
END $$;
