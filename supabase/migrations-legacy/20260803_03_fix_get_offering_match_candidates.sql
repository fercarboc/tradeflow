-- P2.7 FIX: get_offering_match_candidates
-- Bug 1: score ignoraba p_query → UPs encontrados por WHERE tenían score=0 → filtrado por WHERE score>0 → 0 resultados
-- Bug 2: LEFT(descripcion_comercial,20) incluía dimensiones ("80x80") → nunca coincidía en modo auto
-- Fix: añadir puntuación por query (+8 nombre, +4 familia), descripción bidireccional LEFT 15/20
...
BEGIN
  ...
  WITH scored AS (
    SELECT
      ...
      -- Bidireccional: UP en desc del offering O nombre del UP en desc del offering
      ...
      LEAST(100,
        -- Coincidencias exactas de identificadores (alta confianza)
        ...
        -- Coincidencias de clasificación
        ...
        -- Coincidencia de descripción bidireccional (LEFT 15/20 evita incluir dimensiones)
        CASE WHEN up.nombre_canonico ILIKE '%' || LEFT(v_offering.descripcion_comercial, 15) || '%'
             ...
             THEN 10 ELSE 0 END +
        -- Puntuación por query del usuario (fix P2.7: antes era 0 → filtrado)
    ...
    WHERE up.validation_state = 'validated'
      AND (
        -- Modo búsqueda manual: filtrar por query del usuario
        ...
        -- Modo auto (sin query): buscar por campos del offering
...
'P2.7 fix 2026-08-03: score incluye puntuación por query usuario (+8 nombre, +4 familia); descripción bidireccional LEFT 15/20.';
