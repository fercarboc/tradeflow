-- ═══════════════════════════════════════════════════════════════════════════
-- MP-FIN-2C FIX 1/5: actor_id column name + audit 9th param
-- trade_marketplace_orders.actor_id (no provider_actor_id)
-- mkt_fin_audit 9th param: NULL (not text p_source_event_id → jsonb mismatch)
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

-- ── mkt_fin_create_dispute — fix column name + audit 9th param ───────────────
...
BEGIN
  ...
  -- FIX: actor_id not provider_actor_id (column does not exist in trade_marketplace_orders)
  ...
  -- FIX: 4th param still auth.uid() here (fixed in migration 17)
  -- FIX: 9th param NULL (was p_source_event_id — text not castable to jsonb)
  ...
  -- FIX: outbox 3rd param still auth.uid() here (fixed in migration 18)
  PERFORM public.mkt_fin_outbox_publish(
    ...
    jsonb_build_object(
      ...
      'supplier_order_id', p_supplier_order_id,
      'master_order_id',   v_order.master_order_id,
      'provider_actor_id', v_order.actor_id,  -- FIX
      'amount',            p_amount,
      'responsibility',    p_responsibility,
...
-- ── mkt_fin_simulate_dispute_outcome — fix audit 9th param ──────────────────
...
BEGIN
  ...
  IF p_outcome = 'lost' THEN
    ...
    v_entry_row := public.mkt_fin_ledger_append(
      ...
      v_d.currency::char, 'confirmed', now()
    );
