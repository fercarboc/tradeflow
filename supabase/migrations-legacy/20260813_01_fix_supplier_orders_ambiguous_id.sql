-- HOTFIX RC1-C.7: column reference "id" is ambiguous en get_supplier_orders_unified
--
-- Causa: RETURNS TABLE(id uuid, ...) declara "id" como variable PL/pgSQL.
--        WHERE id = p_actor_id (sin alias tabla) es ambiguo → Postgres error 42702.
--        El bug solo se activa cuando _mkt_has_permission pasa (usuario autenticado
--        con orders:read) — de ahí el 400 Bad Request en producción.
-- Fix:   añadir alias "ma" a trade_marketplace_actors y cualificar ma.id.
--        Firma idéntica → CREATE OR REPLACE sin DROP.
...
BEGIN
  ...
  -- FIX: alias "ma" evita ambigüedad con output column "id" de RETURNS TABLE
  SELECT ma.supplier_catalog_id INTO v_catalog_id
  FROM public.trade_marketplace_actors ma
  WHERE ma.id = p_actor_id;

  RETURN QUERY
  WITH all_orders AS (

    -- Pedidos legacy: solo cuando no hay filtro de tienda
    SELECT
      so.id                  AS r_id,
...
GRANT EXECUTE ON FUNCTION public.get_supplier_orders_unified(uuid, text, integer, integer, uuid)
