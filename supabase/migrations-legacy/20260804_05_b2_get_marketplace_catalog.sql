...
-- Devuelve UPs validados con resumen de offerings activas matcheadas.
-- Join correcto: actor.supplier_catalog_id = catalog.id
-- Filtros actor: estado='active' | catalog: is_active=TRUE | offering: activa=TRUE
...
BEGIN
  ...
  WITH active_offerings AS (
    -- Offerings activas con actor y catálogo activos
    SELECT
      o.id,
  ...
  ups_filtered AS (
    -- UPs validados que pasan los filtros activos
    SELECT
      up.id,
      up.nombre_canonico,
      up.descripcion,
  ...
  best_per_up AS (
    -- Mejor offering por UP: stock primero, luego precio, luego plazo
