...
-- A. Extender CHECK modo='api' en trade_catalog_imports
-- B. api_sync_catalog_offerings — upsert de hasta 500 productos vía API
-- C. api_sync_stock            — actualización de stock de hasta 1000 refs
-- D. api_sync_prices           — actualización de precios de hasta 1000 refs
-- E. get_api_catalog_products  — listado paginado (GET /catalog/products)
-- ═══════════════════════════════════════════════════════════════════════════════

-- ─────────────────────────────────────────────────────────────────────────────
-- A. Extender modo para importaciones vía API
-- ─────────────────────────────────────────────────────────────────────────────
ALTER TABLE public.trade_catalog_imports
  DROP CONSTRAINT IF EXISTS chk_import_modo;

ALTER TABLE public.trade_catalog_imports
  ADD CONSTRAINT chk_import_modo CHECK (modo IN ('append', 'api'));

-- ─────────────────────────────────────────────────────────────────────────────
-- B. api_sync_catalog_offerings
--    Upsert batch de productos desde la API externa.
--    Crea un registro en trade_catalog_imports para trazabilidad.
--    Emite outbox catalog.import_completed para matching IA.
--    VOLATILE (escribe); SECURITY DEFINER; solo callable con service_role.
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.api_sync_catalog_offerings(
  p_actor_id      uuid,
  p_items         jsonb,        -- array de objetos producto (máx 500)
...
BEGIN
  ...
  IF p_items IS NULL OR jsonb_array_length(p_items) = 0 THEN
    RAISE EXCEPTION 'INVALID_INPUT: items no puede estar vacío.'
      USING ERRCODE = 'invalid_parameter_value';
  END IF;

  IF jsonb_array_length(p_items) > 500 THEN
    RAISE EXCEPTION 'BATCH_TOO_LARGE: Máximo 500 productos por llamada. Recibidos: %.',
      jsonb_array_length(p_items)
      USING ERRCODE = 'invalid_parameter_value';
  END IF;

  -- Resolver catálogo del actor
  ...
  IF v_catalog_id IS NULL THEN
    RAISE EXCEPTION 'ACTOR_NO_CATALOG: El actor no tiene catálogo activo.'
      USING ERRCODE = 'no_data_found';
  END IF;

  -- Crear registro de importación para trazabilidad
  INSERT INTO public.trade_catalog_imports (
    ...
    p_actor_id,
    COALESCE('api_sync_' || COALESCE(p_source_system, 'external'), 'api_sync'),
  ...
  -- Procesar cada item con SAVEPOINT implícito por fila
  FOR v_idx IN 0..jsonb_array_length(p_items) - 1 LOOP
    ...
    IF v_supplier_ref IS NULL OR v_supplier_ref = '' THEN
      v_errors_arr := v_errors_arr || jsonb_build_object(
        'index', v_idx, 'supplier_ref', null,
        'motivo', 'supplier_ref es obligatorio'
      );
    ...
    IF v_descripcion IS NULL OR v_descripcion = '' THEN
      v_errors_arr := v_errors_arr || jsonb_build_object(
        'index', v_idx, 'supplier_ref', v_supplier_ref,
        'motivo', 'descripcion_comercial es obligatoria'
      );
      v_errores := v_errores + 1;
      CONTINUE;
    END IF;
    ...
    BEGIN
      ...
      SELECT NOT EXISTS (
        ...
        WHERE supplier_catalog_id = v_catalog_id AND supplier_ref = v_supplier_ref
      ) INTO v_is_insert;
      ...
      ON CONFLICT (supplier_catalog_id, supplier_ref) DO UPDATE SET
        ...
        image_url             = COALESCE(EXCLUDED.image_url,
                                  public.trade_marketplace_supplier_offerings.image_url),
  ...
  -- Actualizar contadores en el registro de importación
  ...
  -- Emitir outbox para matching IA si hay productos nuevos
...
-- ─────────────────────────────────────────────────────────────────────────────
-- C. api_sync_stock — actualización de stock por supplier_ref
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.api_sync_stock(
  p_actor_id uuid,
  p_items    jsonb  -- [{supplier_ref, stock_disponible, stock_cantidad?}]
)
RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path = public
AS $$
...
BEGIN
  ...
  IF jsonb_array_length(p_items) > 1000 THEN
    RAISE EXCEPTION 'BATCH_TOO_LARGE: Máximo 1000 referencias por llamada. Recibidas: %.',
      jsonb_array_length(p_items)
      USING ERRCODE = 'invalid_parameter_value';
  END IF;
  ...
  IF v_catalog_id IS NULL THEN
    RAISE EXCEPTION 'ACTOR_NO_CATALOG' USING ERRCODE = 'no_data_found';
  END IF;
  ...
  FOR v_idx IN 0..jsonb_array_length(p_items) - 1 LOOP
    ...
    UPDATE public.trade_marketplace_supplier_offerings SET
      ...
      stock_cantidad   = CASE
                           WHEN v_item ? 'stock_cantidad'
                           THEN NULLIF(v_item ->> 'stock_cantidad', '')::integer
                           ELSE stock_cantidad
                         END,
      updated_at       = now()
    ...
    IF v_rows_affected > 0 THEN
      v_updated := v_updated + 1;
    ELSE
      v_not_found := v_not_found || to_jsonb(v_ref);
    END IF;
  END LOOP;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- D. api_sync_prices — actualización de precios por supplier_ref
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.api_sync_prices(
  p_actor_id uuid,
  p_items    jsonb  -- [{supplier_ref, precio_coste?, precio_venta?}]
)
RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path = public
AS $$
...
BEGIN
  ...
  FOR v_idx IN 0..jsonb_array_length(p_items) - 1 LOOP
    ...
    UPDATE public.trade_marketplace_supplier_offerings SET
      precio_coste = CASE
                       WHEN v_item ? 'precio_coste'
                       THEN NULLIF(v_item ->> 'precio_coste', '')::numeric
                       ELSE precio_coste
                     END,
      precio_venta = CASE
                       WHEN v_item ? 'precio_venta'
                       THEN NULLIF(v_item ->> 'precio_venta', '')::numeric
                       ELSE precio_venta
                     END,
      updated_at   = now()
    WHERE supplier_catalog_id = v_catalog_id AND supplier_ref = v_ref;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- E. get_api_catalog_products — listado paginado para GET /catalog/products
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'plazo_entrega_dias',   o.plazo_entrega_dias,
      'activa',               o.activa,
  ...
  RETURN jsonb_build_object(
    ...
    'pagination', jsonb_build_object(
      ...
      'per_page', p_per_page,
      'total',    v_total,
      'pages',    CEIL(v_total::numeric / p_per_page)::integer
    )
  );
END;
$$;
