-- Fix: api_sync_catalog_offerings usaba gen_random_bytes (extensions schema)
-- con SET search_path TO 'public' → función no encontrada → 500 en Supplier API v1
-- Solución: reemplazar con gen_random_uuid() nativo (PostgreSQL 13+, no requiere extensión)
-- Descubierto durante MKT-FASE1-PILOT-002 ETAPA 6 (2026-08-02)
...
BEGIN
  ...
  INSERT INTO public.trade_catalog_imports (
    ...
    'api_' || replace(gen_random_uuid()::text, '-', ''),
