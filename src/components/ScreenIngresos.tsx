...
export default function ScreenIngresos({ showToast }: Props) {
  ...
  // Datos del gráfico: últimos 6 meses (independiente del filtro de período)
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Ingresos</h2>
          <p className="text-sm text-slate-500">Rentabilidad y facturacion de tu negocio</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5 space-y-1.5">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total facturado</p>
          <p className="text-2xl font-black text-slate-900 dark:text-white">{fmt(totalFacturado)}</p>
          <p className="text-xs text-slate-400">{filtered.length} factura{filtered.length !== 1 ? 's' : ''} en el periodo</p>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-emerald-500/20 rounded-xl p-5 space-y-1.5">
          <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">Cobrado</p>
          <p className="text-2xl font-black text-emerald-400">{fmt(totalCobrado)}</p>
        ...
        <div className="bg-white dark:bg-slate-900 border border-amber-500/20 rounded-xl p-5 space-y-1.5">
          <p className="text-[10px] font-bold text-amber-400 uppercase tracking-wider">Pendiente de cobro</p>
          <p className="text-2xl font-black text-amber-400">{fmt(totalPendiente)}</p>
          <p className="text-xs text-slate-400">
            {vencidas > 0 ? `${vencidas} vencida${vencidas !== 1 ? 's' : ''}` : 'Sin facturas vencidas'}
          </p>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-purple-500/20 rounded-xl p-5 space-y-1.5">
          <p className="text-[10px] font-bold text-purple-400 uppercase tracking-wider">Pipeline</p>
          <p className="text-2xl font-black text-purple-400">{fmt(pipelineTotal)}</p>
          <p className="text-xs text-slate-400">
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1" style={{ height: '100%' }}>
                <div className="flex-1 w-full flex items-end">
                  <div
                    className={`w-full rounded-t transition-all ${hasValue ? 'bg-blue-500 hover:bg-blue-400' : 'bg-slate-200 dark:bg-slate-800'}`}
                    style={{ height: hasValue ? `${Math.max(pct, 3)}%` : '3%' }}
                    title={fmt(m.value)}
                  />
                </div>
                <span className="text-[9px] text-slate-500 uppercase font-medium">{m.label}</span>
              </div>
            );
          })}
        </div>
      ...
      {clientes.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Top clientes por facturacion</p>
          </div>
          {clientes.map((c, idx) => {
            ...
            return (
              <div
                key={c.id}
                className={`flex items-center gap-4 px-5 py-3.5 ${!isLast ? 'border-b border-slate-100 dark:border-slate-800' : ''}`}
              >
                <span className="text-xs font-bold text-slate-400 w-4 shrink-0">{idx + 1}</span>
                <div className="flex-grow min-w-0">
                  <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{c.nombre}</p>
                  <div className="mt-1.5 h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full" style={{ width: `${pct}%` }} />
                  </div>
                </div>
                <span className="text-sm font-bold text-slate-900 dark:text-white shrink-0">{fmt(c.total_facturado)}</span>
              </div>
            );
          })}
        </div>
      ...
      {invoices.length === 0 && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-10 text-center">
          ...
          <p className="text-sm text-slate-400">No hay datos de facturacion aun.</p>
          <p className="text-xs text-slate-500 mt-1">Los datos apareceran cuando crees y cobres tus primeras facturas.</p>
        </div>
      )}
    </div>
  );
