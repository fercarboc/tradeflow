...
import {
  ...
  Banknote, CreditCard, Building2, Smartphone, Camera, Receipt,
} from 'lucide-react';
...
interface Props {
  ...
  isLiveMode: boolean;
  initialMantenimientoId?: string | null;
}

type InvoiceWithClient = TradeInvoice & { trade_clients?: { nombre: string; telefono?: string | null } | null };

export default function ScreenFacturas({ showToast, isLiveMode, initialMantenimientoId }: Props) {
  ...
  const [filterMantenimientoId, setFilterMantenimientoId] = useState<string | null>(initialMantenimientoId ?? null);
  ...
  const filtered = useMemo(() => {
    const result = invoices.filter(inv => {
      if (filterMantenimientoId && inv.mantenimiento_id !== filterMantenimientoId) return false;
  ...
  }, [invoices, filterMantenimientoId, filterEstado, filterSerie, search, dateFrom, dateTo, sortDir]);
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-5xl">
      ...
      {/* Banner: filtro contextual de contrato de mantenimiento */}
      {filterMantenimientoId && (
        <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5">
          <Receipt className="w-3.5 h-3.5 text-blue-500 shrink-0" />
          <span className="text-xs font-semibold text-blue-700 flex-1">Facturas de este contrato de mantenimiento</span>
          <button
            onClick={() => setFilterMantenimientoId(null)}
            className="text-[10px] font-bold text-blue-500 hover:text-blue-700 cursor-pointer flex items-center gap-1"
          >
            <X className="w-3 h-3" />
            Todas las facturas
          </button>
